# Chapter 68 — DOM construction

[Chapter 67](67-html-tokenizer.md) gave us a flat stream of
HTML tokens. By the end of M59, `/bin/htmltok` could walk
...
- **No `id` field on `dom_node`.** `id` is just one of
  ...
  "id")` rather than caching it. The CSS engine in M61
...
- **`malloc` per node, not arena.** A full document is
  hundreds to thousands of nodes; M17 gave userspace a
  32 MiB heap and our test pages don't come close. Arena
...
3. `malloc(sizeof(struct html_token))` heap-allocates the
   ...
   M59. The 16 KiB user stack can't hold a 12 KiB
   automatic.
...
PASS: DOM construction milestone-60 — all checks green
```
...
two sectors / 32 bytes-per-entry = 32 file slots. M59
already filled 32, so M60 hit:
