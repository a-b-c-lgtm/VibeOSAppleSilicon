# Chapter 194 — In-guest Doom rebuild, full vendor compile

> **Milestone in this chapter:** scale the chapter-193 pilot
...
> on Apple silicon. Prerequisites: chapter 193 (pilot),
> 192 (`/bin/make`), 191 (`/bin/tar`), 187 (`/bin/gcc`).
...
1. Write `assets/osfs/doom_full.mk` (~88 LoC): a
   ...
   pattern rule that the chapter-192 engine fans out
...
193 was three small files, picked to exercise the
...
its own address space (chapter 74 CoW + chapter 72 fork),
its own argv/envp slab (chapter 151), its own page table
walk through chapter 91's mmap-backed page cache, and
exits cleanly through chapter 77's SIGCHLD/waitpid path.
...
replaced by the osdev shim (deferred to 195). The
...
Same shape as 193's pilot, scaled up. The OBJS list is
...
The chapter-192 pattern-rule engine handles this without
...
chapter-162 recipe executor. No special-casing for the
absolute paths.

## Pitfalls

### Pitfall — `/bin/make` buffer sizes from 192 are too small
...
**Cause:** The 192 version of `/bin/make`
([`userspace/make/make.c`](../../../userspace/make/make.c))
had:
...
whitespace). The 192 reader silently truncated the line
...
After the bump, every existing chapter-192 test still
passes (`test_make_v2.py` 9/0). The 194 full compile
runs cleanly.
...
| `test_tar.py` | 8/0 | |
| `test_make_port.py` | 14/0 | chap-162 toy makefile |
| `test_gcc_hello.py` | 10/0 | |
| `test_gcc_stdio.py` | 7/0 | |
| `test_gcc_bf.py` | 6/0 | |
| `test_gcc_sys_stat.py` | 6/0 | |
| `test_doom_pilot.py` | 8/0 | chap-193 |
...
chapter-174 host-built doom ever did, because every `.c`
...
- **ctype:** `isdigit`, `isspace`, `isalpha`, `tolower`,
  `toupper` — from 167.
- **setjmp:** `setjmp`, `longjmp` — from 165.
...
The fundamental loop from 193 — tarball → tar → make →
...
- **`assets/osfs/doom_full.mk`** (new, ~88 LoC including
  ...
  Absolute paths everywhere (193 trap), `-DNORMALUNIX`
...
- **`userspace/make/make.c`** (edited): bumped
  ...
  width (still 512). Existing chapter-192 tests pass
  unchanged.
...
desktop" milestone lands in 195–196 when the link
step produces a binary and the WAD ships.

## What's next

**Chapter 195 — link `doomgeneric.elf` in-guest.**
Known blockers:
...
3. **`/bin/ld` capacity** — the chapter-155 in-guest
...
**Chapter 196 — rebuilt doom plays.** Re-run the
chapter-174 smoke test, but with the in-guest-built
