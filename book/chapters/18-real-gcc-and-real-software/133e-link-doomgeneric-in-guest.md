...
Chapter 133d compiled 77 vendor `.c` files in-guest into
...
*own* object files. Up to chapter 133d the largest thing
...
what `/bin/ld` does in-guest). The chapter-131f binutils
...
- The POSIX-idiomatic answer is **binutils' `@file`
  ...
  *opens the file* and splices each whitespace-separated
  ...
  command-line args.
- The chapter-131f `/bin/ld` is real GNU binutils 2.44,
...
OSFS-1 (chapter 27 / 132j) has a hard 19-byte filename
...
The chapter-133d `/bin/make` buffer sizes (MK_MAX_VAL=4096,
...
in-guest (chapter 133d showed exactly that). To get a
...
in-guest do the interesting work" pattern chapter 130a
used for the cross-built Doom reference binary. The point
of 133e isn't to re-test the compile (133d did that). It's
to test the link.

`doom_link.args` lists **82** objects. `doom_full.mk`
(chapter 133d) lists **80**. The two missing in 133d are
`gusconf.o` and `icon.o` — a drift between fixture and
host build that 133d quietly tolerated because it only
needs to compile, not link. The link step exposes the
drift as undefined references (`mus_pitch`, the icon
glyph table). For 133e, both paths get added to the args
...
The chapter-122 book entry noted this as "DAMAGE failed
...
**Cause:** OSFS-1 (chapter 27) packs filenames into a
fixed 19-byte field in the directory entry.

**Fix:** Renamed to `doomobjs.tar` (12 bytes). All four
133e fixtures verified under the limit before commit. The
...
includes both `gusconf.o` and `icon.o`; the chapter-133d
...
**Fix:** For 133e, add `/data/src/gusconf.o` and
`/data/src/icon.o` to `doom_link.args`. The symmetric fix
to `doom_full.mk` (so the in-guest 133d compile produces
the same 82 objects 133e expects) is queued for 133f.
...
**Cause:** This is a chapter-119 carry-over: the linker
...
that's a chapter-119 follow-up, not a 133e fix.
...
**Cause:** The chapter-116 `/bin/wc` is the simple
...
regression sweep (133d's 25-min compile run on demand).
...
produced binary is the next step (chapter 133f); the
...
Chapter 133f — run `/data/doomgeneric.elf` inside the
guest. Open questions:

1. Does it find a WAD? Chapter 130b staged
   `/bin/doom1.wad`; the in-guest binary will need to
   open that through the chapter-117 stdio (`fopen`,
   `fseek`, `fread`).
...
`doom_full.mk` so a fresh in-guest 133d compile would
produce all 82 objects 133e expects to find.
