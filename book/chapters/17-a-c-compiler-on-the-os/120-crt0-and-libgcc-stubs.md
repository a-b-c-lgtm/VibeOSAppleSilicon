...
[Chapter 119](119-bin-ld-linker.md) gave us `/bin/ld`.
...
[chapter 121](121-bin-cc.md), and any future GCC port
...
we expect to compile in chapter 121+:
...
matches `printf.h`, `malloc.h`, and the chapter 116
...
compatible with our `/bin/ar` from chapter 119.
...
Chapter 119 already taught us that any filesystem opting
into exec needs a `load` op in its `fs_ops` vtable
(returning `long` — not `int`, see the chapter-119
memory note). Chapter 120's atexittest doesn't poke at
tmpfs — but if you ever try to `cp atexittest /tmp/foo
&& /tmp/foo`, the chapter 119 fix is what makes that
...
- **Existing apps:** every binary in `userspace/`
  ...
  chapter 119 — `_user_start` does the trivial path
...
- **Existing tests upgraded:** none required. The full
  ...
  (`test_atexit` + the 18 from chapter 119).

## What gets exercised in tests

```
[chapter 120] /bin/atexittest smoke test
PASS: ctor1 ran
PASS: ctor2 ran
PASS: main ran
PASS: exit1 ran
...
[Chapter 121 — `/bin/cc`](121-bin-cc.md) is the first
