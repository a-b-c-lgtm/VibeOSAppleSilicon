...
long vfs_stat_path(const char *path, struct kstat *out) {
     ...
     * dispatch vfs_lseek went through in chapter 116b. */
...
Before chapter 117, `ls /path` ran a complex
...
Chapter 116d's 15-test baseline + the new
`test_libc_stat.py`:
...
- `getcwd` — chapter 33 maintains `PWD` in env; the few
...
- **The `is_dir` vtable method already paid for itself.**
  ...
  implemented `is_dir` for chapter 85's subdirectory work.
...
Chapter 118 ships `/bin/as` — the AArch64 assembler — and
