# Chapter 86 — Subdirectories: a path walker, mkdir, and a navigable Save As

OSFS-2 has had directory-shaped inodes since chapter 82.  Look
...
The notepad in chapter 85 saves to `/data/<name>`.  That works
...
- Browser cookies (chapter 96-ish), which want
...
2. **Storage cost.**  `.` + `..` in every directory is two
   ...
   stays as it was in chapter 82: every dirent is a real
   thing.
...
Before chapter 86, `osfs2_create` did three things:
...
The existing `SYS_LISTDIR` (chapter 19) was designed when
...
chapter 86 navigation test uses pixel-counting on the cream
...
chapter 85's `test_notepad_save_as.py`.
...
- **Cross-mount mkdir.**  `mkdir /tmp/foo` returns EINVAL
  today.  Tmpfs is flat by design (chapter 31) and adding
  hierarchy there is a separate feature, not a "while
  we're here" detour.
- **A path component cap > 60.**  OSFS-2's dirent name
  field is 60 bytes (chapter 82); chapter 86 inherits that.
