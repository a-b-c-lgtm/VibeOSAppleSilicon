# Chapter 112e — TLS trust store (multi-anchor)

Chapter 112d wired one trust anchor into `/bin/browser`: the
...
Chapter 112e fixes both. The trust store becomes a fixed-size
...
- `scripts/mkcabundle.py` — host-side tool that extracts the
  ...
  (chapter 112f will add PEM parsing for the Mozilla NSS root
  list).
- `userspace/browser/browser.c` — adds a one-shot
  ...
  falls back to the chapter-112d in-binary single anchor when
...
In chapter 112e the bundle carries two anchors:
...
- `/bin/httpsd 8443` (chapter 112b) keeps presenting the
  RSA-2048 sample chain.
- `/bin/httpsd --ec 8444` (chapter 112e) presents the ECDSA
  P-256 sample chain.
...
The chapter-112d anchor baker assumed `pk->key.rsa.{n,nlen,e,elen}`
...
- `userspace/libc/tls_socket.{h,c}` — multi-anchor refactor;
  ...
  outside, so chapter-112c's `/bin/tlstest --handshake-ca`
...
- `scripts/test_browser_https.py` (unchanged) — proves the
  bundle path doesn't regress the chapter-112d single-anchor
...
- `scripts/test_tls_handshake.py` (unchanged) — proves the
  knownkey path (chapter 112b) survives the struct layout
  change.
