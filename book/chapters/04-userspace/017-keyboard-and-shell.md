# Chapter 17 — Console keyboard input and a line-mode shell
...
After chapter 16 the kernel had `init`, `spawn`, `wait`, and exit
...
We have all the pieces (chapter 8 GIC, chapter 11 wakeups). What we
...
— the `SP_EL0` save we added in chapter 16 means each thread has
...
Compare to chapter 16's discussion of fork-vs-spawn. We're building
...
ran earlier, then hands control to the shell.
...
- Many shell features (argv, PATH, pipes, signals, history) are
  deferred. None of them are blocked by anything we built here;
