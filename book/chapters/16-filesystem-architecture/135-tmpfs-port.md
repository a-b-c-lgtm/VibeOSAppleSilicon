# Chapter 135 — Step 3: porting tmpfs onto `fs_ops`

After chapter 134 proved the adapter shape works for a
...
```c
/* Inside vfs_open, after the procfs vtable dispatch from 134: */
...
fires in chapter 139; today, on /tmp (which isn't RO), we
pass through and the create succeeds.

### `sys_listdir_at` is the one syscall this step doesn't touch

The legacy unified `sys_listdir` (chapter 19) still walks
...
*is* re-routed through the vtable (chapter 134 set this up
...
- procfs (regression from 134)
- subdirectory creation under `/data` (chapter 86)
- notepad save-to-`/data` round-trip (chapter 85)
- clipboard daemon using tmpfs-backed scratch files (chapter 113)
- named IPC service registration (chapter 112, indirectly
  exercises tmpfs)
...
- Existing apps using the feature: `clipboardd` (chapter 113)
