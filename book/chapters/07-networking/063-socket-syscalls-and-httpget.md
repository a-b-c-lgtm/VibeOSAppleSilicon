# Chapter 63 — Socket syscalls and a userspace `httpget`

[Chapter 62](062-tcp-and-sockets.md) ended with a TCP client
...
kernel's TCP self-test (~5 seconds on this machine).
...
3. Wait for the kernel's TCP self-test to finish (this also
...
through the new socket syscall surface. There is no shortcut.

## What we deliberately did not do

A few things that look conspicuously absent:

- **No `bind()` / `listen()` / `accept()`.** Server side comes
  later. The browser only needs the client side.
...
- **No real `select()` / `poll()`.** The spin-yield loop in
  ...
  multiplexing eventually, but not yet.
...
Each of these is a fine future improvement; none of them
block "fetch a URL" semantics.

## What this unlocks

With socket syscalls in place, we have, end-to-end:
