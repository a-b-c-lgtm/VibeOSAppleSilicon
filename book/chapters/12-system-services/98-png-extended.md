# Chapter 98 — Extending the PNG decoder: palette, grayscale, and content-type sniffing

Chapter 97 shipped a PNG decoder that could read the assets *we*
...
2. **Decode what we recognised.** The PNG in the bug report is a
   400×400 paletted image — a colour format chapter 97 did not
...
Chapter 97 supported only the rightmost two (8-bit RGB and 8-bit
...
2. **Pre-decode and cache.** `br_img_cache_install_bytes()` is a
   peer of the existing `br_img_cache_load()` from chapter 97,
...
commit policy from chapter 97 stays in force: zlib output isn't
...
the picture. The next ladders — JPEG (chapter 99 candidate), GIF,
...
most satisfying micro-arcs in the project. Chapter 97 *built* the
infrastructure — cache, layout seam, blitter, tests — out of
nothing, and chapter 98 *spent* it: every new format is now a
