# Chapter 96 — A real RTC and wall-clock time
...
The taskbar clock has been ticking since chapter 55, but until this
...
Chapter 96 wires the QEMU `virt` machine's PL031 RTC into the
...
operation chapter 96 performs is a single 32-bit read at
`base + 0x00`.
...
SMP-safe from chapter 93). No syscall dispatch overhead beyond
the inevitable SVC.

The downside: we don't pick up host-clock adjustments after
boot. If the host's NTP daemon steps the clock by 0.5 s during
runtime, we miss it. That's acceptable for chapter 96's floor —
...
we'd handle that with a periodic re-snapshot. For chapter 96
...
`strftime_iso` is the only formatter we need for chapter 96
...
* **No file mtimes or ctimes.** OSFS-2 (chapter 81) will be
...
* **Bench output stamps.** The browser's
  `--bench-resize` mode (chapter 95) currently prints
...
* `scripts/test_rtc.py`
* `book/chapters/12-system-services/096-rtc-and-wallclock.md`
  (this file)
