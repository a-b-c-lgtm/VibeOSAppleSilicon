...
> binaries stay buildable for readers of chapters 154 and 119
> but are no longer wired into the disk image. Prerequisite:
> chapter 179 (cross-built `ld-new` + `as-new`).
...
The chapter-154/155 toy `as` and `ld` were
single-mnemonic-table, single-section, no-relocation tools.
They carried the book through chapters 157 (`/bin/cc`), 162
(`/bin/make`), 163 (notepad Build button) and the
chapter-172 Doom port — but they cannot link binutils,
gcc, or doomgeneric's full feature set. To advance to
chapter 181 onwards ("real gcc in-guest") a real
linker is the prerequisite.

Chapter 179 built `ld-new` and `as-new` from
...
**Cause:** `build/disk.img` was 16 MiB (chapter 158
...
comfortable headroom for chapter 181 onwards.
...
**Cause:** the chapter-155 toy `ld` hard-coded the
...
`mrs xN, tpidr_el0` instructions. The chapter-156 crt0
...
169). When `ld-new`'s `lexsup.c` called `getopt_long`,
...
**Fix.** Extend chapter 179's `OSDEV_LIBC_NO_GETOPT`
...
(per chapter 179's libc-bridge pattern), which calls
...
- **`/bin/cc`** ([userspace/cc/cc.c](../../../userspace/cc/cc.c))
  ...
  Same `chapter 158` toolchain contract.
...
- **notepad Build button** (chapter 163) unchanged in
...
- Existing tests upgraded to the GNU pipeline:
  ...
  (docstrings rewritten to call out chapter 180 as
  the contract).
...
Chapter 181 brings up GNU `gcc-14.2.0` against the
same `aarch64-osdev` target triple. The libc-bridge
pattern from 179 and the `cstring.o`-as-LDADD trick
