# Chapter 183 — Cross-building `aarch64-osdev-gcc`
...
> resolution through the chapter-175 binutils prefix.
> Prerequisites: chapter 175 (binutils-osdev), 181 (patched
> gcc source), 182 (in-tree math libs).
...
5. Verify with `scripts/test_xgcc_build.py` that the
   ...
   through the chapter-175 binutils prefix.

---

## Why now

Chapters 181 and 182 put the patched source and the
...
those chickens-and-eggs belong in chapter 184's
...
- `gmp/`, `mpfr/`, `mpc/` — host libraries, built with
  ...
  compiler proper links against. Chapter 182 put them in
  place via symlinks.
...
- `libgcc/`, `libatomic/`, `libssp/`, `libstdc++-v3/`,
  ...
  headers. That's why chapter 183 stops at
  `make all-gcc`.
...
**`--target=aarch64-osdev`** — the triple chapter 181
...
as the chapter-175 binutils. This is what wires gcc to
binutils: when gcc looks for `as` and `ld` at runtime it
searches `$prefix/$target/bin/` and `$prefix/bin/`
*before* `$PATH`, and that's exactly where chapter 175
...
stops. That's enough to satisfy chapter 183's contract:
...
(`-o foo`) doesn't, but that's chapter 184's contract,
not 183's.
...
full 4 minutes. That'll matter in chapter 183-follow-up
...
| `-dumpversion` = `14.2.0`           | the source tree pinned in 181      |
| `__osdev__` / `osdev` predefined    | proves `TARGET_OS_CPP_BUILTINS` from `aarch64-osdev.h` fired during the build (chapter 181's hook is now active) |
| `-print-prog-name=as` points into `build/toolchain/` | proves gcc found the OSdev binutils via the shared `--prefix` trick |

If any of those fail, chapter 183's contract is broken
...
Chapter 182 wired the `$(GCC_PREREQS_MARKER)` into
...
Chapter 184 will introduce a second manual target,
...
- Unchanged: chapters 175 (binutils-osdev), 181
  (gcc_target), 182 (gcc_prereqs) smokes all still PASS
  after this chapter's build.

## What this unlocks

- New host build target: `make gcc-osdev` (Makefile, in
  the new chapter-183 block after `clean-gcc-prereqs`).
