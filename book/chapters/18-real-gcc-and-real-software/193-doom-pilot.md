# Chapter 193 — In-guest Doom rebuild, the pilot
...
> Prerequisites: chapter 187 (`/bin/gcc`), 190 (`sys/`
> headers on OSFS-1), 191 (`/bin/tar`), 192 (expanded
> `/bin/make`).
...
So 193 is the pilot: same loop, but with three small
...
`SYS_CHDIR` (chapter 23). But the compile then
failed:
...
Chapter 193 picks **Option B**. The cwd fix is queued for
...
`/bin/doomgeneric.tar` already ships (chapter 191). All
...
`<inttypes.h>` cleanly. The chapter-192 pattern
expansion synthesised three rule instances correctly. The
chapter-191 tar extraction landed files where expected.
...
| `test_make_v2.py` | 9/0 |
| `test_tar.py` | 8/0 |
| `test_make_port.py` | 14/0 (chap-162 toy makefile still works) |
...
1. Scale OBJS from 3 to 82 files (chapter 194), iterating
   on whatever libc gaps surface.
2. Add a link step (chapter 195) to produce a binary —
   which requires shipping `libgui.a` on `/bin/` so the
   osdev shim links cleanly.
3. Stage the WAD (chapter 196) so the binary has
   something to render.
...
in 194–195 when an actual `doom` binary appears.

## What's next

Chapter 194 — scale to the full 82-file vendor compile.
...
- the FILE\* abstraction from chapter 150 (stdio gaps)

Then 195: link `doomgeneric.elf` in-guest. Requires
