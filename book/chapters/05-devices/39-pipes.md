# Chapter 39 — Kernel pipes: ring buffers, blocking I/O, and `dup2`
...
later.  When pipe traffic gets heavy enough to feel it (chapter 50,
...
Recall from chapter 38 that SVC handlers run with all DAIF bits set
...
in different threads), and that has to wait until chapter 40 when
...
condition: socket recv (chapter 39 of the next book), waiting for a
...
- `kernel/core/main.c` — banner bumped to milestone 30.
