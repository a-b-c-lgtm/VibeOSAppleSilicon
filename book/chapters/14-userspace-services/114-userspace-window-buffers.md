...
* Chapter 91 — mmap + the bump-pointer allocator
  we reuse for the user VA range.
* Chapter 74 — COW. The skip-on-fork mechanism
  here echoes the one chapter 74 added for COW.
* Chapter 72 — fork's eager-copy path, which we
...
* **New debug helper:**
  ...
  — full serial dump for chapter-114 bring-up.
...
* A path to GPU acceleration: a future
  ...
  stays the same.
* Resizable mapped windows (a future chapter 116): the
