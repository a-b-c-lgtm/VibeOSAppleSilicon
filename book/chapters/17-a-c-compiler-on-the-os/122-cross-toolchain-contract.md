# Chapter 122 — Cross-building from the host: the contract

## Why this chapter exists

[Chapter 121](121-bin-cc.md) shipped a tiny native
compiler at `/bin/cc`. Chapter 123 will grow it from
...
- `as` and `ld` matching its calling convention (we
  ...
  [118](118-bin-as-assembler.md) and
  [119](119-bin-ld-linker.md))
...
3. **Chapter 125:** name the gap between `/bin/cc` and
   a self-hosting compiler explicitly, and stop. A real
   GCC port (Part XVIII) inherits the contract this
   chapter wrote down.

This is the same shape we used for chapter 121 — pick a
...
(see [Chapter 17](../03-mmu/17-user-heap.md)), so any
...
undocumented — chapter 122 is also where it gets
...
| Sysroot of standard headers | Our headers are deliberately minimal (chapter 116); a real GCC port expects POSIX |
| `-O2` semantic equivalence | We currently build at `-Os`; opt-level changes belong to whichever backend we adopt |

## Next

[Chapter 123 — `/bin/cc` grows variables and arithmetic](123-cc-variables-and-arithmetic.md)
