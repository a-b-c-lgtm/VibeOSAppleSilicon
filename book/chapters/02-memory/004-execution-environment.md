...
- **EL0** is where every application will eventually run. We get to
  EL0 in chapter 14, when we ELF-load the first user program.
...
3. **`x30` (LR) is callee-saved by convention, not by hardware.**
   ...
   pattern is the source of the early stp/ldp gotcha you met
...
yet. Userspace is a different story — chapter 13 brings the FP/SIMD
...
bit) to enable IRQs in chapter 8 and `DAIFSet, #2` to disable
