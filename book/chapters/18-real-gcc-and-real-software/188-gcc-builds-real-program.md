# Chapter 188 — `/bin/gcc` builds a medium real program
...
> / FAIL 0**. Prerequisite: chapter 187 (default-spec link
> works).
...
```
[chapter 188] /bin/gcc rebuilds bf from source
...
name. The default specs from chapter 187, plus the on-disk
...
187 was about) and definitely does not add to the
...
already shaken out by chapters 184–187.
...
- That `make` works in the guest. (Chapter 162 already
...
- **Existing app modified:** `/bin/gcc` is exercised
  against a new, larger source than chapter 187's
  `int main(void) { return 7; }`.
- **New test:** `scripts/test_gcc_bf.py`. PASS 6/6.
- **Existing tests still green:** `test_gcc_hello.py`
  (chapter 187) still passes 10/10 — adding bf to the
...
- Chapter 191 ports `make` so multi-file programs
  compile without hand-driving `gcc` one TU at a time.
- Chapter 192–f drive a full upstream port through the
