# Chapter 163 — Notepad's Build button: closing the loop inside the guest
...
manually compiled with `/bin/cc` (chapter 160).  Chapter
...
notepad     write_buffer_to("/tmp/np_src.c")
   ...
   │            │
   │            │   /bin/cc reads /tmp/np_src.c (chapter 160)
...
1. **Apps must use the OS features we built.**  Chapter
   126 shipped `/bin/make`.  If chapter 163 bypassed it,
   the make port would be a dead artefact in the disk
   image rather than a real piece of the user-visible
   toolchain.  Every kernel/userspace feature has to be
   exercised by at least one real application; routing
   the build button through `/bin/make` is what makes the
   make port a real piece of the toolchain rather than
   shelfware.
...
| `.PHONY`, automatic variables, multi-file projects | Future make extensions (chapter 164+) |
...
- **`fsync`** (chapter 83) — used by both `write_buffer_to`
  ...
  durable.
- **Notepad** (chapter 31, then chapters 46/84/118) —
...
- A POSIX-ish libc rich enough to host a small compiler
  pipeline (chapters 148, 117).
- An in-guest assembler, linker, and `ar` (chapters
  118, 119).
- A crt0 + libgcc-style runtime stubs (chapter 156).
- A from-scratch ~1000-line `/bin/cc` accepting a tiny
  subset of C \u2014 literals, locals, `int` arithmetic, a
  fixed-shape `printf`, `return`/`exit` (chapters 157,
  123).
- A regression-gated cross-toolchain contract that any
  future backend has to satisfy (chapter 158).
- The first on-disk native compile (chapter 160).
- An honest write-up of the self-hosting gap that
  Part XVII does *not* close (chapter 161).
- `/bin/make` (chapter 162) and notepad's Build button
  (this chapter).
...
- **A real GCC port.** The cross-toolchain contract in
  chapter 158 is the spec a `aarch64-none-osdev-gcc`
  would build against; chapter 161 catalogues the
