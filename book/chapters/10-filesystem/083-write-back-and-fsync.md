# Chapter 83 — Write-back, fsync, and the durability gap

Chapter 82 made OSFS-2 writable.  Every `echo hi > /data/x`
...
We already have `blk_cache` (chapter 22).  Why not extend it?
...
That invariant let chapter 22 keep the eviction code trivial
...
can lose it:
  ...
  3. **Cooperative shutdown that doesn't call sync first.**
     The 'reboot' path in chapter 11 would lose the cache.
...
`notepad`.  Two changes in `userspace/notepad/notepad.c`:
  ...
  2. **fsync before close.**  In `save_file`:
     ...
     /* Chapter 83 — force the write-back cache to disk before
...
- The journal in chapter 84 will wrap the same fsync /
...
Chapter 84 fixes this with a tiny write-ahead journal in a
