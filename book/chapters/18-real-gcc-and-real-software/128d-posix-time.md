...
1. Replace chapter 95's placeholder `<time.h>` (which used a
...
3. Declare `difftime` as `static inline` knowing it can't be
   *called* until chapter 129 lifts `-mgeneral-regs-only`.
...
Chapter 95 shipped a placeholder `<time.h>` using a non-POSIX
...
POSIX shapes. Chapter 128d swaps in the POSIX surface and
...
Chapter 128d replaces it with the C99 shape:
...
(set at the top of the Makefile until chapter 129), the
...
Once chapter 129 lifts `-mgeneral-regs-only`, the existing
...
Same as 128c. Any OBJS block defined above `CSTRING_OBJ` in
...
- **130a (Doom platform shim)** — `time(NULL)` for the RNG
  seed, `clock()` for frame timing.
- **131c (binutils)** — `mktime`/`gmtime` for archive member
  timestamps.
- **132e (GCC)** — `localtime` + `strftime` for `-time` and
  diagnostic output.
- **Chapter 129** — wires `difftime` and the FP path through
