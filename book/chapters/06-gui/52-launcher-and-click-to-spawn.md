# Chapter 52 — launcher: clicking is the new typing

> *Milestone 44.  Up to now, every GUI app on this system has had
...
> buttons that `spawn()` the binaries we built in chapters 49,
...
its own fresh address space (chapter 24) and a brand-new
window from `gui_create_window` (chapter 48).  The launcher's
...
make this look better immediately.  The chapter 49 `wm_focus`
...
2. QEMU's tablet device synthesises evdev events into the
   virtio-input ring.
3. Our virtio-input driver (chapter 49) decodes them, scales
...
6. The kernel's spawn syscall walks `/bin/gui_term` from OSFS
   (chapter 21), allocates an address space (chapter 24),
   loads the ELF (chapter 13), creates a thread, and schedules
   it.
