# Chapter 29 — `uptime` and a real shell PATH
...
interrupt back in chapter 10). Each tick is `TICK_INTERVAL_MS`
...
have env vars yet (chapter 21+ work) so we hardcode the lookup:
