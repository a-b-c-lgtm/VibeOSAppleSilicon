...
This is the same shape as chapter 131e (cross-build `ld`):
...
EABI, built with the chapter-131b wrapper and the
chapter-131e env (`-mcpu=cortex-a72 -DNDEBUG
...
132a) and `vendor/binutils-aarch64-osdev.patch` (chapter
131a) is the original; these three are mechanical copies
...
This matters because chapter 132e was developed by
...
**Cause:** chapter 131e's pattern for the binutils
...
The chapter-116b `userspace/libc/stdio.h` defined exactly
one guard:
...
The shared env settings come from chapter 131e:
...
- `-DOSDEV_LIBC_NO_GETOPT` — chapter 131d guard so each
...
shell script sourced before any probe runs.  Chapter 131e
populated it with ~40 `${ac_cv_*=value}` entries; chapter
132e added zero new ones.  All three of gmp / mpfr / mpc
...
build re-runs many times during 132e+ debugging without
...
Worth recording because it's evidence the chapter 131d/e
...
- **The aarch64-osdev-cc wrapper.**  No edits.  Whatever
  ...
  ignores.
- **The libosdevc.a archive form** (chapter 131e).
  GMP's configure runs link probes; the wrapper's
  `--start-group / --end-group` plus the chapter-131e
  `cstring.o` extern bridge cover everything.
- **crt0.o** (chapter 120).  GMP's configure runs trivial
...
- **The 8 unprefixed binutils symlinks** in the sysroot
  (chapter 132d).  gcc's driver finds `as` / `ld` /
...
- `book/chapters/18-real-gcc-and-real-software/132e-gcc-prereqs-for-guest.md` (this file)
- `book/INDEX.md` (+ 132e link)
...
| `python3 scripts/test_xgcc_compile.py` (132d regression) | PASS |
...
- **Existing app touched**: none yet.  The end-state
  consumer is the chapter-127 notepad's "Build" button,
  which currently shells out to the chapter-131b wrapper
...
Until then, chapter 132e's payoff is the three archives
...
3. **The gcc tarball ships its own libtool plugin
   build** (`liblto_plugin.la`).  Chapter 131e's
...
4. **A second cstring.o batch.**  gcc's host code calls
   ...
   that 131e didn't cover.  Add asm-renamed wrappers as
