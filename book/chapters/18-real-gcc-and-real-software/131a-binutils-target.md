...
GCC in chapter 131c. GCC's target-triple controls:
...
- The default library search paths and crt0 names. On a
  ...
  already shipped at chapter 120.
- The libc and the default `--with-newlib` / `--with-glibc`
  ...
  *is* one here (chapters 116–128), and the cross compiler
  needs to know that.
...
- `targ_emul=aarch64elf` — reuse the existing aarch64 ELF
  ...
  text base (0x400000 for static binaries — chapter 13).
...
aarch64-osdev-objcopy  — section copier (needed in 131c for crt files)
...
- **131b** — port `aarch64-osdev-as` and
  ...
  the host ones.
- **131c** — GCC. The big one. Re-uses every flag this
...
- **131d** — swap `/bin/as` and `/bin/ld` (currently the
  chapter-118 and chapter-119 stubs) for the real binutils
  ports from 131b. The chapter-121 `/bin/cc` driver will
...
Chapter 131b lifts the host-only `aarch64-osdev-gcc`
