# Chapter 94 — Sharing the FD table: CLONE_FILES and refcounted fd_table
...
> chapter 92 (`SYS_CLONE`) and chapter 93 (`SYS_CLONE2`).

Chapter 92 put thread spawning into userspace (`SYS_CLONE`) and
chapter 93 made spawning honour CPU placement (`SYS_CLONE2`).
...
Chapter 94 fixes the rule by lifting the fd table into a
...
* **Chapter 88** — atomics. `fd_table::refcount` uses
  `atomic_add_return32` / `atomic_sub_return32` exactly the same
  way `address_space::refcount` does (chapter 92).
* **Chapter 92** — CLONE. `SYS_CLONE3` is a strict super-set of
...
* **Chapter 93** — CLONE2. The CPU-placement field on
...
Chapter 94 introduces the indirection:
...
```c
/* kernel/core/thread.h — chapter 94 */
struct thread {
    /* ... id, sp, state, name, etc ... */
...
Before chapter 94 the kernel had 138 references to `t->fds[`,
...
chapter 92's address-space refcount uses — `address_space_share`
is the prior art:
...
internalising. It's the same property that lets chapter 92's
...
chapter 93's `user_thread_create_shared_on` with one extra
parameter:
...
sibling next to chapter 93's `thread_spawn_on`:
...
This is the only function chapter 95 will reach for directly.
...
```
PASS: chapter 94 CLONE_FILES smoke test
```
...
* **No `CLONE_VM` / `CLONE_SIGHAND` / `CLONE_FS`.** Today
  ...
  AS by construction" (chapter 92 set this up); cwd / env
...
* `scripts/test_clone_files.py`
* `book/chapters/11-smp-and-memory/094-clone-files.md` (this file)
...
in the chapter 59+ regression set.
