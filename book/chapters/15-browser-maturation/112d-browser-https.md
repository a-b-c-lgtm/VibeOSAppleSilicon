# Chapter 112d — Browser https:// (native TLS)

Chapter 112c built a chain-validating TLS client and proved it
...
Python TLS-terminating proxy from chapter 106a — a development
prop that lived outside the guest and silently rewrote URLs
through `BROWSER_PROXY`. Chapter 112d wires the chapter-112c
...
- `scripts/test_browser_https.py` — boots the kernel, drives
  ...
  visible in the boot log, the new chapter-112d TLS-OK line
  from the browser, and the decrypted response tokens reaching
  the renderer.
- `book/chapters/15-browser-maturation/112d-browser-https.md`
  (this file) and the corresponding `book/INDEX.md` rows.

`scripts/https_proxy.py`, the host-side TLS terminator from
chapter 106a, stays in the tree per
`/memories/debug-scripts-policy.md` — we keep every debug
script for future reference — but no regression script
launches it any more. The browser no longer needs it.

## The br_conn_t abstraction

The fetch path before 112d worked on a single integer:
...
into a "TCP" and a "TLS" variant, 112d introduces a tagged
union:
...
chapter-112d test screens for `browser: TLS read error` in
its fail-fast list precisely so we notice if this branch ever
fires unexpectedly.

## The single-anchor trust shape

112d still uses the chapter-112c sample chain
...
--handshake-ca` proved out in 112c. Chapter 112e takes the
...
two production users of the old rewrite both pre-date 112d and
...
`/bin/httpd` forwarder we built in 106b. That path is `http`,
not `https`, and is unaffected by 112d.
...
| `userspace/httpsd/httpsd.c` | Unchanged. Same chapter-112b response body, same boot launcher in `init.c`. |
...
| `scripts/https_proxy.py` | Unchanged on disk. No longer launched by any regression script. Retained per `/memories/debug-scripts-policy.md`. |
...
- `scripts/test_browser_https.py` — the new 112d regression.
- `scripts/test_browser_self.py` — unchanged from chapter
  108b, proves the bare browser path still works.
- `scripts/test_browser_proxy.py` — unchanged from chapter
  106b, proves the legacy `BROWSER_PROXY` rewrite still
  works (so the gate doesn't accidentally block existing
  users).
- `scripts/test_tls_chain.py` — chapter 112c regression,
  proves `tls_socket_init_chain_from_anchor` still works in
  isolation.
- `scripts/test_tls_handshake.py` — chapter 112b regression,
  pinned-key handshake.
- `scripts/test_tlstest.py` — chapter 112a link/run smoke.
...
- The chapter-106b host proxy is now genuinely optional. It
...
`localhost` shortcut from 112d stays as the loopback path for
