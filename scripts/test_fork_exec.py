#!/usr/bin/env python3
"""scripts/test_fork_exec.py — boot kernel, run /bin/forktest at the
...
SOCK = "/tmp/osdev-serial-forktest.sock"
...
def main():
    ...
    try:
        ...
        # Extract just the forktest output for readability.
        idx = log.rfind(b"forktest\r\n")
        if idx < 0: idx = log.rfind(b"forktest\n")
        section = log[idx:].decode("ascii", "replace") if idx >= 0 \
            else log[-2000:].decode("ascii", "replace")
        print("--- forktest output: ---")
        print(section)
