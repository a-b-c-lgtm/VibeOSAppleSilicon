# Chapter 47 — virtio-input: an evdev keyboard for the GUI

Chapter 46 gave us pixels.  This chapter gives us a keyboard.
...
raw-TTY line editor, and the SIGINT plumbing of chapter 37 all
...
delivery on `Ctrl-C` (chapter 37) just works.
...
2. **If a window has focus, the GUI session consumes the byte and
   ...
   exclusively to the focused GUI app in chapter 48.  The shell
...
WM test in chapter 48.

## Three gotchas

1. **`memset` from struct initialisers** (again — see chapter 46).
...
- **virtio-tablet (mouse)** — same device id, separate instance
  ...
  or by EV_ABS / EV_KEY capability bits.  Coming in milestone 41.
