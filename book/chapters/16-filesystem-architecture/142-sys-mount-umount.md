# Chapter 142 — Step 2: `SYS_MOUNT` and `SYS_UMOUNT`

[Chapter 141](141-kernel-userfs-module.md) landed the
...
[chapter 143](143-libfs-and-echofs.md). What we get here
...
exists; chapter 143's `test_userfs_echo.py` covers them
end-to-end.
...
landed in 141; this is the first call site that fills the
...
`userfs_op_open` (141) and decremented by `userfs_op_close`
(141 + the `fd_table_unref` close loop), so it tracks the
...
Chapter 132 didn't need a removal API because the in-kernel
...
chapter 146's hardening pass adds an exit hook to
...
- **Tests upgraded**: none. The kernel-table change is
  ...
  (chapter 138), which still passes despite the new
  mount-removal code path being added.
- **Tests added**: none in this chapter — see 143 for
