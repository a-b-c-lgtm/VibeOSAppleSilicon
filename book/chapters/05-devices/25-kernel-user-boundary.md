...
Per-process address spaces (chapter 24) gave each user process its
...
above DRAM, see chapter 24), but every other slot was *inherited*
from the boot L1 with the same `BLOCK_NORMAL_USER` permission.

Once chapter 24 moved the user range to slot 64 (above all DRAM),
...
Pre-milestone-16 this silently succeeded. Now:
...
kernel/core/main.c              banner bumped to milestone 16
...
change. That's the whole of milestone 16.
