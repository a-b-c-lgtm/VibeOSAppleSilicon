# Chapter 98 — PNG decoding and the browser image cache
...
- Chapter 62 — the browser pipeline (fetch → tokenize → DOM →
  CSS → layout → paint).
- Chapter 61 — layout, especially the `LAY_BOX_REPLACED` path and
  the paint command stream.
- Chapter 27 — `gui_present` syscall (used to blit the BGRA buffer
  into the browser's window).
- Chapter 49 — the BGRA pixel format used everywhere in the
...
WOFF2 font reader in chapter 99.
...
`pip install --user pillow` with the PEP-668 escape hatches).
...
- **Chapter 99 (TrueType fonts).** WOFF2 wraps a TrueType
...
- **Chapter 101 (procfs).** `/proc/self/maps` will compress
