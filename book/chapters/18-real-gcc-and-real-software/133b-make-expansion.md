...
4. Add `scripts/test_make_v2.py` (4 steps, 9 expectations:
   ...
   run) and verify it lands 9/9 with chapter 126's
...
Chapter 126 shipped a `/bin/make` whose entire vocabulary was:
...
The standing user directive from
[`/memories/apps-must-use-features.md`](../../memories/apps-must-use-features.md)
says: when the OS adds a feature, existing apps must use it.
`/bin/make` is the app; the feature it needs is the same
feature every real Makefile uses.
...
The Doom build doesn't need those. Chapter 133c ships a
...
The recipe-run path is unchanged from chapter 126:
...
cpp finds them via `-isystem /bin` — chapter 132j convention.
...
| Test | Result |
| --- | --- |
| `test_make_port.py` (chapter 126) | 14/0 |
...
Chapter 126's toy makefile still works because the simple
...
Chapter 133c can now write a hand-tailored Makefile for the
...
- Existing app rewritten to use the new feature: **none yet
  ...
  app calls it. Chapter 133c uses it from the in-guest Doom
...
- New test scripts added:
  [`scripts/test_make_v2.py`](../../../scripts/test_make_v2.py)
  — 9 expectations, the chapter-133b regression.

## What's next

Chapter 133c uses the expanded `/bin/make` from inside the
guest to drive the DoomGeneric build: extract the tarball
from chapter 133a onto `/data/src/`, drop a tailored
