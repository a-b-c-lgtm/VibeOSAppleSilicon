# Chapter 87 — The second core: PSCI and secondary boot
...
The actual scheduling work happens in chapter 90. For now, our
...
- Chapter 6 — MMU and page tables (the secondary inherits the
  boot CPU's L1).
- Chapter 10 — Threads and the AArch64 context switch (we'll
  reuse the stack-pointer discipline in `secondary_start`).
- Chapter 8 — GIC v3 (we don't program the redistributor for
  CPU 1 until chapter 89, but it's worth glancing at).
...
per-CPU runqueue (chapter 90), a per-CPU IPI mailbox (chapter
...
test go red the moment we landed chapter 87. The line now
...
chapter 89 lands — the IRQ handler would spin forever waiting
...
the same userspace behaviour as before chapter 87.
...
- `scripts/_dbg_smp_boot.py` — chapter 87 smoke test.
