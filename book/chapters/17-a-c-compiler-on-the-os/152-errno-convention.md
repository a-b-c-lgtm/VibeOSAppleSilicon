# Chapter 152 — POSIX `errno` convention + `strerror` + first FILE-* app port
...
chapter 150 put in place, proving that the libc surface is
...
Up to chapter 149 the convention was the opposite: `open()` returned
`-ENOENT` on failure, and the value was *also* mirrored into
`errno`. That dual-channel design was a deliberate
backward-compat shim — chapter 7 to chapter 147 had ~99
caller sites of the shape `printf("err=%d\n", -fd)`, and the
errno chapter (149) made the new path opt-in.

Chapter 152 removes the shim:
...
in (149), the FILE-* layer (150), and the env arena (151)
without touching every existing app the same week. By the
time chapter 153 needs `stat` / `dirent` / `getcwd` shaped
...
function. The `errno` plumbing from 149 does the rest.
...
- The convention shim was the right call. Without
  149's "negative-rc *and* errno" dual-channel design,
...
[Chapter 153 — libc, part 2: `stat`, `fcntl`, `dirent`,
`getcwd`](153-libc-stat-fcntl-dirent.md). The next
