# Chapter 55 — A taskbar, three new GUI syscalls, and a real desktop

By milestone 46 the system boots straight to a desktop and shows a
...
and no visible representation of the WM's window list.  Milestone 47
adds all three.
...
`compose_all` now does two passes, both using the milestone-45
...
- **Two bits of metadata buy a desktop.**  `NO_DECORATION` and
  ...
  flags will let us add a clock and a notification bubble in M48
...
- **Decouple semantically-distinct counters.**  The cascade counter
  ...
  Same lesson as M45: when two invariants share state, separate them.
