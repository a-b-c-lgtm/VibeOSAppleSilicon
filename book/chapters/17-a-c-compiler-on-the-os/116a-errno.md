# Chapter 116a — Step 1: errno, populated by every syscall wrapper

Chapter 116 promises a POSIX-shaped libc: real `FILE *`, real
`errno`, real `setenv`. That's enough work that breaking it
into three landings — `116a` errno, `116b` stdio, `116c` env
...
The chapter 116 strategy doc calls for the wrappers to switch
...
So 116a does the half that's reversible and risk-free: errno
...
from "return negative" to "return `-1`" is deferred to 116d,
...
app that exercises it. For 116a:
...
- **Existing-app rewrite, deferred:** the 20-odd
  ...
  116d flips the wrappers to `-1 + errno`, these sites
...
- **Upcoming chapters:** 116b's `FILE *` layer reads
  `errno` to populate `ferror`; 116c's `getenv` rewrite
  uses `EINVAL` on bad input. Without this sub-chapter
  neither layer would have a real errno to talk to.

## What is *not* done yet (the 116b/c/d work)
...
- **No per-thread errno.** `__errno_value` is process-global.
  ...
  upgrade — chapter 91's TLS slot is the obvious home for it
...
- **No wrapper convention flip.** Every wrapper still returns
  `-errno` on failure. The flip is 116d's job.
- **No `FILE *`.** That's 116b.
- **No `setenv` / `putenv`.** That's 116c.
- **No `strerror`.** A trivial helper but unneeded until 116d
  switches `perror`-style sites away from the `-fd` idiom.

## Side-effects for next chapters

- 116b can now write `f->error = errno;` after a failing
...
└─ 116 (libc: stdio, errno, env)
   ├─ 116a  errno foundation  ← this chapter
   ├─ 116b  FILE * + buffered I/O
   ├─ 116c  setenv / putenv / environ arena
   └─ 116d  -1 + errno convention flip + app migration
```

After 116d, chapter 117 (stat / fcntl / dirent / getcwd)
