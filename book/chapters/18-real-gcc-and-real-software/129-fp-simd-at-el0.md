...
4. Fill in the eight `stp d*, d*` / `ldp d*, d*` pairs in
   `userspace/libc/setjmp.S` that chapter 128a reserved space
   for.
...
- `difftime` (chapter 128d) becomes real instead of a stub —
  it really does subtract `time_t`s and return a `double`.
- `setjmp` / `longjmp` (chapter 128a) finally save the AAPCS64
...
3. **AArch64 FP register file is 512 bytes.** Sixteen 32-byte
   ...
   hasn't been anywhere near the slab limit since chapter 92.
...
```asm
    /* Chapter 129 — enable FP/SIMD at EL0 and EL1.
...
during PSCI bring-up (chapter 86). After this point, every EL0
...
```asm
    /* Chapter 129 — save FP/SIMD state.  Per AAPCS64 every
...
```asm
    /* Chapter 129 — restore FP/SIMD state before GPRs.
...
Chapter 128a's `setjmp.S` only handled the integer set,
...
132i lands the full umbrella headers on disk.
...
- **`difftime` (chapter 128d)** — was a stub returning
...
- **`setjmp` / `longjmp` (chapter 128a)** — now AAPCS64-
  compliant for FP-using callers. Doom's `I_Error` recovery
  path uses `setjmp`; the previous chapter-128a version
...
Chapter 130a — cross-build a working Doom on the host
toolchain. Vanilla Doom sources, `aarch64-elf-gcc` cross,
output an ELF that *would* run in our guest if it could
read its own WAD. (130b adds WAD reading via the existing
`open`/`read` syscalls; 130c brings the framebuffer.)
