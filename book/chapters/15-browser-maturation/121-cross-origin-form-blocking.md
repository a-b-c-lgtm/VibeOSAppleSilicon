# Chapter 121 — Cross-origin form submission blocking

[Chapter 120](120-cookies-and-sop.md) shipped the cookie half
...
codebase: there's no `<base href>` (chapter 132+ territory)
and no JavaScript (chapter 122+), so a relative action can
only resolve to the same origin as the page. The check is
defense-in-depth — it costs ten lines now and means the
chapter 122 author doesn't have to remember to wire SOP
themselves when `form.action = ...` becomes assignable from
JS.

### 3. SOP compares the *raw* URL, not the proxy form

A subtle bug found during bring-up: chapter 110's HTTPS
...
does it; the harness is 200+ lines). For chapter 121 we
...
- **`<base href>` parsing.** Without it the blocked branch
  is unreachable in production. Chapter 132 ships this.
...
- **Redirect-following SOP.** If the action's host
  ...
  redirects at all yet — chapter 110 explicitly punts.)
...
2. The test harness mirrors
   ...
   121 follow it keeps the book's test surface uniform.
...
- Existing book wiring:
  - [book/INDEX.md](../../INDEX.md) — chapter linked under
    Part XV.
- Tests added:
  - [scripts/test_browser_sop.py](../../../scripts/test_browser_sop.py) —
    8 PASS / 0 FAIL.
- Tests still green (verified post-change):
  ...
  - [scripts/test_browser_cookies.py](../../../scripts/test_browser_cookies.py)
    — chapter 120 round-trip still passes, confirming the
...
- Chapter 122's pocket JavaScript can ship `form.action`
