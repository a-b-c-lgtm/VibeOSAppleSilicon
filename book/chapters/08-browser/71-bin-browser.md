# Chapter 71 — `/bin/browser`

[Chapter 70](70-block-and-inline-layout.md) ended with `/bin/layout`
...
- `/bin/httpget` (M56–M58) — to fetch pages over our own TCP/IP
  stack and our own URL/HTTP parser, with one-hop redirects
  resolved against the origin.
- `htmltok` (M59) → `dom` (M60) → `cssparse` (M61) →
  `layout` (M62) — the rendering pipeline, run end-to-end on
  whatever HTML/CSS comes back.
- The window manager (M40, M45, M51) — to host the resulting
...
|------|------|---------------|
| `--paint` | dumps the paint stream verbatim | regression diff against M62 fixtures |
...
matcher in M61 takes `(user_agent_css, author_css)` and walks
the DOM emitting matches; the layout engine in M62 turns those
...
Up through milestone 51 the WM created windows with two
behaviours: drag from the title bar, and click the `×` to close.
M62 added z-order and minimize. None of these supported a *user-
...
Real browsers reflow on resize for the same reason. M62's layout
...
- **Image fetching.** `<img>` lays out at its `width`/`height`
  ...
  one M-of-its-own (a JPEG/PNG decoder library plus a sprite
  cache).
- **Forms.** The DOM knows about `<a href>` and `<input>`;
  layout knows about anchor underlines. Anchor *clicks* are
  wired up as of milestone 64 (see the postscript below);
  forms still aren't.
...
## Postscript (milestone 64): toolbar, history, and clickable links

The M63 browser of this chapter could only display whatever
...
Milestone 64 adds the missing UI without touching any of the
...
its limit (see chapter 27's postscript). The hit-tester walks
...
`load_page` runs the full M59-M62 pipeline (fetch + tokenize
...
- The first attempt to render a real-world page (HN's index)
  ...
  persist-timer interaction — see [chapter 63](../07-networking/63-tcp-and-sockets.md)'s
  postscript.
- The first click into the comment thread segfaulted in
  `css_match_chain` because the user thread stack was 16 KiB and
  the document nests 50 levels deep — see [chapter 27](../05-devices/27-argc-argv.md)'s
  postscript.
