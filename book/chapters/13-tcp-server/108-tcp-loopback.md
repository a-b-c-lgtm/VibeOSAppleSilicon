# Chapter 108 — TCP loopback (lo0 and 127.0.0.0/8)
...
in this chapter, and chapter 111 gets to demo the clean
...
frame to `virtio_net_tx`. Chapter 108 inserts one branch
...
and we'll probably hit it again in 109 when httpd starts
...
chapter 108 was meant to deliver.
...
- **Chapter 109** -- httpd serving over a TLS bridge.
...
- **Chapter 110** -- the browser's `BROWSER_PROXY` env
  var (or equivalent) finally has a stable in-guest
  address to point at: `127.0.0.1:8080`.
- **Chapter 111** -- the hermetic browser-to-httpd test.
...
- [Chapter 60](../07-networking/060-ethernet-arp-ipv4.md) --
  where `net_ipv4_send_from` was first introduced.
- [Chapter 62](../07-networking/062-tcp-and-sockets.md) --
  `tcp_tx`'s checksum machinery.
- [Chapter 105](105-passive-open-listen.md) -- the listener
  + child-conn pattern that loopback exercises.
- [Chapter 106](106-accept-and-server-sockets.md) -- the
  `socket_accept` syscall the looptest parent uses.
- [Chapter 107](107-bin-httpd.md) -- the predecessor
