# Chapter 69 — Block and inline layout

[Chapter 68](068-css-parser.md) ended with `cssparse` printing
...
- a **paint command stream** — back-to-front list of `RECT`,
  `TEXT`, and `UNDERLINE` ops that a renderer (the browser binary or any
...
[BOX#4]   kind=TEXT  x=16  y=16  w=48  h=22  text="Demo"
...
[PAINT#1]   TEXT       x=16  y=16  w=48  h=22  color=#FF000080 fs=32 fw=700 fst=0 "Demo"
...
Everything that the renderer in the browser binary will need is now decided in
this layer. The browser just consumes the paint stream.
...
The cascade is the only stage that needs help from the CSS matcher.
...
layouts that are easy to test and easy to reason about. When the
browser binary gets a real GUI framebuffer we can swap in proper metrics.

(Chapter 104 partially addressed this: the bitmap font was
...
The `/bin/browser` capstone will tolerate them: most static content on
the open web renders well enough at this fidelity to read.

## Defensive engineering: freestanding `mem*`

A trap that bit us partway through this chapter: GCC's optimiser silently
...
`/bin/layout` produces a paint stream. The `/bin/browser` capstone
