...
1. Write `scripts/test_doom_rebuilt_plays.py`: a
   ...
   `test_doom_link.py` (chapter 133e) as a
...
3. Sample the same 240×200 region inside the cascade-
   ...
   `scripts/test_doom_plays.py` uses (chapter 130c) so
...
This is what the chapter-128 plan called "the
end-state". The arc that started at chapter 128a
...
+ GCC porting, and the five-chapter 133a–e "real
...
(chapter 133e) as a Python `subprocess.run`. That test
...
- `V_Init:` — Doom's renderer says "video initialised".
  ...
  `/data/doom1.wad` via the on-disk stdio (chapter 117),
...
math matches `test_doom_plays.py` so chapter-130c's
...
build-time fixture step, like 133e shipping
...
- **Byte-equivalence to the cross-built reference.**
  The chapter-130a cross-built `/bin/doom` and the
  chapter-133e in-guest-linked `/data/doomgeneric.elf`
...
- **Input handling.** The test doesn't send any
  ...
  interactive-feel territory; not chapter 133f's scope.
- **Audio.** Doom is configured with `-mb 6` (memory
  ...
  binaries run silent. Chapter 96's virtio-snd stack
...
1. **`doom_full.mk` / `doom_link.args` symmetry.**
   Chapter 133d's full-compile manifest lists 80 files;
   chapter 133e's link manifest lists 82 (adding
...
2. **A native `gcc` driver invocation that produces a
   ...
   compile to `.o` (chapter 132h) and the in-guest
   `/bin/ld` can link `.o`s to an elf (chapter 133e),
...
- ELF load from OSFS-2 (chapter 113)
- argv plumbing through sys_spawn (chapter 91)
- vfs_load via osfs2 against `/data/*.elf` (chapter 81)
- stdio fopen against `/data/doom1.wad` (chapter 117)
- wmclient IPC against wsd (chapters 108a–e)
- wsd compositor + scanout (chapters 108d, 109d)
...
This is the last chapter of Part XVIII. The chapter-128
...
chapter 112d's https client could fetch
...
130c), so the "apps must use features" requirement is
...
(133g — real key-up events) closing a deferred input
issue that chapter 130c flagged as out-of-scope: the
chapter-130b timed-release shim gets replaced with real
