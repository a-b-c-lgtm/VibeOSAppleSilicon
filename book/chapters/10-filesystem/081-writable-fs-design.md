# Chapter 81 — Why we need a writable filesystem
...
That has been fine for chapters 20–79.  The system could be
...
nicknamed **OSFS-2**.  Chapters 82–85 implement it
incrementally.
...
Each of these maps directly to one of chapters 82–84:
...
- Trivially extends with a journal in front (chapter 84).
...
purposes once chapter 90's mmap shows up.
...
kernel driver.  Once we have a journal (chapter 84) it grows
...
struct osfs2_inode {
    ...
    uint32_t size;          // bytes
    uint32_t nlink;         // hard link count (always 1 in chapter 82)
    uint32_t mode;          // permission bits, cosmetic for now
    uint32_t ctime_ms;      // creation time (uptime ms; chapter 92 will use real time)
...
records.  Lookup is linear scan.  In chapter 82 the root
directory is one block (64 entries max); subdirectories are
not implemented.  Chapter 83 grows the root to multiple
...
filesystem and the chapter 20 narrative continues to refer to
them.

OSFS-2 mounts at **`/data/`** alongside `/mnt/` and `/tmp/`.
Notepad, shell history persistence, and browser cookies will
all be migrated to `/data/` in chapter 85.  Long-term plan:
...
though chapter 82 won't ship an fsck, knowing what it would
...
Chapter 83 shows that "consistent" is hard to maintain
across a crash without a journal.  Chapter 84 ships the
journal.

## What this chapter unlocks

Nothing yet — this chapter is design only.  Chapter 82
...
- The "what does fsck have to do after a crash" framing:
  ...
  a single line of it.
- Why the journal can be left for chapter 84 instead of
  designed into the format from day one.

## Prerequisites

- Chapter 20 — OSFS-1 read-only mount.
- Chapter 22 — block cache (in front of virtio-blk).
- Chapter 19 — virtio-blk driver.

## What chapters 82–85 will deliver

- **Chapter 82.**  `mkosfs2.py` host tool.  Second virtio-
