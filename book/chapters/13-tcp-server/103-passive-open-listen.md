...
syscall is chapter 104, and a real userspace HTTP server is
chapter 105. Chapter 103 is the kernel-internal foundation
those two will sit on.

## Prerequisites

- [Chapter 63 -- TCP client + state machine](../11-networking/63-tcp.md):
  the existing `tcp_connect` / `tcp_handle` / `tcp_poll`
  pipeline, the conn-table layout, the segment TX path.
- [Chapter 56 -- Sockets and httpget](../11-networking/56-sockets-httpget.md):
...
`tcp_accept` with `yield()` (chapter 104) so it doesn't burn
...
The same lesson holds for userspace once chapter 104
...
- **Chapter 104** -- a userspace `accept()` syscall that
  wraps `tcp_accept` and integrates with the fd table.
- **Chapter 105** -- a tiny `/bin/httpd` that listens
  on port 80 and serves files out of the on-disk
  filesystem.
- **Chapter 106** -- end-to-end: a userspace HTTP server
...
- **Multiple-listener semantics** -- one listener per
  port, full stop.
- **A real `accept()` syscall** -- chapter 104.
