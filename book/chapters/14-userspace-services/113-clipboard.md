# Chapter 113 — The system clipboard, as a userspace service
...
thanks to chapter 112's IPC primitive — the fix lives entirely in
userspace.

This chapter ships three things on top of chapter 112:
...
| **us, before chapter 112** | **kernel WM** | would have been kernel |
...
to live in the kernel today. Before chapter 112, the
...
With chapter 112 in hand, that's no longer true. The
...
chapter 112's per-message ceiling (`SRV_MSG_MAX` =
...
(chapter 112); the "keep the daemon alive" policy
...
it, not to back off. When chapter 132 adds the audio
...
3. `clip set Hello chapter 113` → daemon logs `SET
   gen=1`.
4. `clip get` → "Hello chapter 113" comes back on the
   serial console.
...
- **Shell-level clipboard for pipelines.** `echo
  ...
  you can type.
- **Chapter 132 audio mixer.** Same `supervise()`
