# Chapter 61 — Ethernet, ARP, and IPv4

[Chapter 60](60-virtio-net.md) ended with a driver that could put
...
- Static IPv4 configuration: `10.0.2.15/24`, gateway
  `10.0.2.2`. DHCP is [chapter 62](62-icmp-udp-dhcp.md).
...
`virtio_net_set_rx_callback()` from chapter 60. `net_init()`
...
hang forever. The same trick was used by the milestone-52
self-test in chapter 60.
...
chapter 62's UDP and ICMP code will want to compose IPv4
...
The hand-rolled 42-byte ARP frame that lived in `main.c` for
milestone 52 is gone. In its place, a much shorter test that
...
- **ICMP**, so we can't ping yet. That's [chapter 62](62-icmp-udp-dhcp.md).
...
[chapter 62](62-icmp-udp-dhcp.md).
