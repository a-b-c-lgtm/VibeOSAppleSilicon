# Chapter 138 — Step 6: `SYS_MOUNTS` and `/bin/mount`
...
`SYS_MOUNTS = 95` (slot picked because chapter 123's
...
- **Per-field zero, not `= {0}`.** Freestanding C with
  ...
  The fix is to zero the prefix in an explicit loop — the
  same workaround the rest of the freestanding userspace
  applies whenever it would otherwise emit an implicit
  `memset` against a missing symbol.
...
- **Bounded copy.** The kernel's `m->prefix` is a string
  ...
  protects us the day chapter 140 lets userspace coin its
...
`GETRAND_ELF` / `GETRAND_STRIPPED` from chapter 123).
...
- Existing apps using the feature: none yet — `mounts()` is
  the new API; chapter 140 (userspace fs servers) will be
