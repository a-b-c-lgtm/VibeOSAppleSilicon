# Chapter 134 — Step 2: porting procfs onto `fs_ops`

Chapter 133 landed the empty mount table. This chapter takes
the smallest existing filesystem — chapter 101's procfs — and
pulls it across to the new vtable. We pick procfs first for
exactly the reason the [plan chapter](132-mount-table-and-vtable.md)
...
chapters 135–137 adopts the same `_strip_slash` helper for
...
enough. Chapters 135–137 port their drivers' open path
...
A full sweep ran green too: chapters 101, 100, 47, 32, 30, 90 all
...
- New apps added: none — the user-visible payoff lands in
  138 when `/bin/mount` becomes possible.
