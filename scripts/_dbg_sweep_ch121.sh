...
mkdir -p "$(dirname "$LOG")"
cd "$(dirname "$0")/.."
TESTS=( test_cc_hello test_atexit test_bin_as test_bin_ld_ar
  test_libc_stat test_libc_errno test_libc_stdio test_libc_env
  test_boot_to_desktop test_userfs_echo test_clipboard test_mount_ro
  test_userfs_timeout test_httpd_forward test_browser_proxy test_cow
  test_fork_exec test_busy_on_mix test_clone_files test_directories )
pass=0; fail=0; failed=""
: > "$LOG"
for t in "${TESTS[@]}"; do
  pkill -f qemu-system-aarch64 2>/dev/null; sleep 1
  echo "===== $t =====" >> "$LOG"
  out=$(timeout 240 python3 scripts/$t.py 2>&1 | tail -60)
  echo "$out" >> "$LOG"
  if echo "$out" | grep -qE "SUMMARY:.*0 FAIL|^OK$|^ALL PASS"; then
