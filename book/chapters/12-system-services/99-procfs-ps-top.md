...
- [Chapter 16 — Files, VFS, and a tiny ramfs](../04-userspace/16-files-and-vfs.md)
  ...
  pipes (chapter 30), tmpfs (chapter 32), tcp sockets
  (chapter 55), ptys (chapter 79b), the writable OSFS-2 mount
  (chapter 84), and now `/proc/`.
- The `SYS_LISTDIR_AT` syscall, introduced alongside the
  `/data` mount, is what `ls /proc` and `/bin/ps` use to
  enumerate the pseudo-FS.
- [Chapter 89 — SMP runqueue](../11-smp-and-memory/89-smp-runqueue.md)
  established the per-CPU runqueues that `/proc/sched` now
  reports the depth of.
- [Chapter 93 — refcounted fd_table](../11-smp-and-memory/93-clone-files.md)
