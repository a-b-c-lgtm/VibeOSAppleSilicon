# Chapter 16 — `init`, `spawn`, `wait`: the simplest process model that works
...
[Chapter 15](015-files-and-vfs.md) ended with two user programs (`hello`
...
Today our kernel still uses **one global page
...
chapter 21 (when we get to multi-tasked workloads in earnest)
will revisit.
...
parsed as ELF by the loader from chapter 14, and handed to
