# Chapter 108e — Decoration, the cursor, input routing, and resize

Chapter 108d retired the kernel compositor. The
...
something worse than chapter 48.
...
chapter 108d cutover left the kernel still in
...
than living as a sub-phase of 108d.
...
`libgui/draw.h` from chapter 108c — the same
...
chapter 108d: paint the FB at `(x, y)`,
...
3. Draws the title text via `draw_text`
   ...
   chapter 108b for glyph cache lookups).
...
briefly interrupted and belongs conceptually to chapter 107's
...
108d also keeps passing — the cascade
...
* **A real input migration
  ([`/srv/input`](../14-userspace-services/107-ipc.md)).**
