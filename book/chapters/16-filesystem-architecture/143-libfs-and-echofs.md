# Chapter 143 — Step 3: `libfs` and `/bin/echofs`

Steps 1 and 2 ([141](141-kernel-userfs-module.md),
[142](142-sys-mount-umount.md)) put the kernel half in
...
chapters 144 and 145 (real daemons: clipboardd, procd)
...
chapter 144) would be to copy the protocol decode loop out
...
- **Reading the cap from `flags` on READ.** The chapter-141
...
`= {0}`, which GCC would lower into an implicit `memset`
call the freestanding userspace can't satisfy) and
...
chapter 16 handles compilation; we don't need a custom rule.
...
mirrors the chapter 138 mount test in structure: boot,
...
been latent since chapter 10 — invisible because no
...
[chapter 144](144-clipboardd-port.md), which rewrites
