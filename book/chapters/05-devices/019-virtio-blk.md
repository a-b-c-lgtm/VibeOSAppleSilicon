# Chapter 19 — virtio-blk and persistent storage

This chapter takes the virtio-mmio framework from chapter 18 and
...
```make
$(DISK):
	@printf 'OSDEV virtio-blk sector 0\n' > $(DISK).sector0
...
Once we set the `Status` bits up to `FEATURES_OK` (chapter 18),
...
The two `dmb` barriers matter (see chapter 18 for the
...
Chapter 20 will introduce virtio-console next — the same transport,
