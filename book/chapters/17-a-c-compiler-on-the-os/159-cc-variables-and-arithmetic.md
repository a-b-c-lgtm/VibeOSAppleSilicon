# Chapter 159 — `/bin/cc` grows variables and arithmetic

## Why this chapter exists

[Chapter 157](157-bin-cc.md) shipped a one-page compiler.
...
[Chapter 158](158-cross-toolchain-contract.md) wrote down the
...
Chapter 157's grammar grew exactly four things:
...
`/bin/as` (chapter 154) encodes `mov` between registers as
...
- **`/bin/cc` itself is the app.** Chapter 157 shipped it
  as a printf-only stub. Chapter 159 turns it into the
...
- **No existing app modified** — the compiler doesn't
  need to be used by another app yet. Chapter 160's
  test will compile a real `hello.c` *from disk* (not
  from a string baked into a Python harness), and
  chapter 163 wires `/bin/cc` into notepad's Build
...
Toward a real compiler — chapters 160+ will bite off
...
[Chapter 160 — First native compile: `hello.c` from
`/data/src/`](160-first-native-compile.md) takes the
