# Chapter 173 — Staging the WAD (so Doom actually plays)
...
> 172 (Doom port), chapter 82 (OSFS-2 layout).
...
3. Move the WAD path the shim defaults to from
   ...
   today; subdirectories land in chapter 86).
...
## Why now — what 172 left on the table

172 got `doom.elf` built, linked, loaded, and executing all
...
```gitignore
# Chapter 173: optional Doom IWAD asset, drop your own copy at
...
the same way it did at the end of chapter 172. The
...
`/data/wads/doom1.wad` — that's where the chapter 172
shim's hard-coded default argv pointed. But OSFS-2, as
introduced in chapter 82, has exactly one directory: the
...
```python
# scripts/mkosfs2.py (chapter 82-vintage)
...
manipulation that the kernel will get in chapter 86. That's
...
- Extend `mkosfs2.py` with `mkdir` support (a chapter of
  work). Defer to chapter 86, where the kernel learns
  subdirs too.
...
When chapter 86 lands and the kernel can walk subdirs, the
...
on OSFS-2 without double-indirect; chapter 82's bound is real
...
The DATA_DISK rule in chapter 82 was deliberately
...
The 173 replacement uses a `wildcard`-guarded prerequisite
...
Chapter 172's `userspace/doom/doomgeneric_osdev.c::main`
...
is untouched from 172. A user can still override on the
command line:
...
entry for Doom (there isn't yet — chapter 174 material).

---

## Testing the difference

`scripts/test_doom.py` from 172 treated *any* DOOM banner OR
the "Game mode indeterminate" message as PASS. After 173
...
`D_IdentifyVersion` both succeeded. Before 173 you'd never
...
- **`malloc(6 MiB)` for the zone allocator** — exercises the
  user heap plus `sbrk()` growth. The
  zone is `-mb 6` (megabytes) in the shim default; chapter
  172 settled on this size, and the upstream default of 16
...
- **`fopen` / `fread` on `/data/doom1.wad`** — first
  ...
  from chapter 83 caches the indirect block but each
  4 KiB data block lookup still goes through OSFS-2's
  bmap path. Worth knowing when chapter 90's mmap lands:
...
- **The window from `DG_Init`** — paints once with `wm_present`
  ...
  in chapter 117 is in the boot path of every doom run,
...
the keyboard (via the chapter-172 `DG_GetKey` press+release
...
without touching the Doom port: chapter 174 (desktop
launcher entry so Doom can be started from the start menu),
chapter 131 (audio — DG's `I_StartupSound` is currently
stubbed, and chapter 97's virtio-snd is wired and waiting).

---

## Postscript — the in-game movement bug

> **Update (chapter 197):** the timed-release shim described
> below was retired in chapter 197 once real `GUI_EVENT_KEY_UP`
> events were plumbed through `virtio_input.c` → `wm.c` →
> userspace. The shim is preserved here as the historical fix
> that closed chapter 173's manual-play acceptance gate;
> after chapter 197 the doom shim consumes release events directly
...
> game loops); read chapter 197 for the architecturally
> correct fix.

The first time the chapter-173 build is run interactively,
...
The chapter-172 shim emitted a synthetic release back-to-back
with every press, because chapter 29's input layer drops key
...
keeps the chapter-173 origin story; once chapter 197
