# Chapter 157 — `/bin/cc`: a one-chapter native C compiler
...
The plan-of-record in [Chapter 147](147-c-compiler-strategy.md)
...
syscalls shipped in
[chapter 77](../09-process-model/077-sigchld-and-waitpid.md).
...
[chapter 156](156-crt0-and-libgcc-stubs.md). The
...
[chapter 154](154-bin-as-assembler.md) handles a
...
This bug had never fired before because chapter 155's
...
```
/* generated by /bin/cc — chapter 157 */
.text
.global _user_start
_user_start:
    bl   .LSjmp0
...
- **Existing apps:** none changed. `/bin/cc` is a
  ...
  anything yet. Chapter 160 will be the first user.
- **New apps:** `/bin/cc` itself.
- **New tests:** `scripts/test_cc_hello.py`. Added
  to the chapter 157 regression sweep alongside
  every test from chapter 155–156.
...
The plan for the rest of Part XVII is: chapter 158
locks down the cross-toolchain contract; chapter 159
grows `/bin/cc` with locals and arithmetic; chapter 160
relies on an on-disk source file for the first real
native compile; chapter 161 is an honest accounting
of the self-hosting gap that Part XVII does not close.
A real GCC port is left for Part XVIII.

## Next

[Chapter 158](158-cross-toolchain-contract.md) —
