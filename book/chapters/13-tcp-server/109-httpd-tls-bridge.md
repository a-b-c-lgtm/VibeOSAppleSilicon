# Chapter 109 — httpd as a forwarding proxy (TLS bridge)
...
Chapter 107 gave `/bin/httpd` exactly one job: serve files out of
...
The motivation is downstream: chapter 110 wants the browser's
...
- [Chapter 108 -- TCP loopback](108-tcp-loopback.md) -- so
  the eventual chapter-110 setup (browser dials
  `127.0.0.1:8080`, httpd dials `10.0.2.2:8080`) doesn't
  share a netdev between the two TCP conns.
- [Chapter 57 -- URL and HTTP parser](../07-networking/065-url-and-http-parser.md)
...
validate path, serve. Chapter 109 inserts ONE branch in
the middle:
...
if (is_local_path(target)) {
    ...
    long n = serve_forward(cfd, raw, raw_len);        /* ch 109 */
...
inside httpd's per-request handler. With chapter 108's
...
non-loopback destination. But chapter 110 will repoint
...
is exactly what chapter 108 bought us.

## The 4 KiB request cap

Chapter 107 sized `HTTPD_REQ_CAP` at 2 KiB, on the
reasoning that our test-harness GETs are short. Chapter
109 bumps it to 4 KiB because the forward path replays
...
behaviour chapter 107 had; we just chose a different
boundary.
...
```
Phase A -- forward path (chapter 109)
...
Phase B -- local path regression (chapter 107)
...
None of those are chapter-109 work. They're chapter-15X
...
- **Modified app:** [`userspace/httpd/httpd.c`](../../../userspace/httpd/httpd.c)
  gains the chapter-109 forward path. ~90 new lines, all
...
- **Chapter 110** -- the browser's `BR_DEFAULT_PROXY`
...
- **Chapter 111** -- the end-to-end celebration. Two of
