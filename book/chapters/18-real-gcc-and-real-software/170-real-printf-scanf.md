# Chapter 170 — Real `printf` and `scanf`

> **Milestone in this chapter:** grow the libc formatter from
> the chapter-19 minimum to the full C99 conversion-spec
...
> that scans a config file. Prerequisites: chapters 18 and
> 150 (the existing printf and `FILE *` infrastructure),
> chapter 169 (stdlib).
...
`%f` on the scanf side) are deferred to chapter 171 — they
...
1. **Output side.** Chapter 18's `printf.h` (extended through
   150) already covers `%d`, `%i`, `%u`, `%x`, `%X`, `%p`,
...
chapter 171 alongside the FP-at-EL0 work.
...
struct already has a one-byte `ungot` slot (chapter 150), so
...
A pre-existing bug from chapter 18's printf, cleaned up at
...
`stdlibtest` (chapter 169): `CHECK()` macro increments
...
- **`userspace/date/date.c`** doesn't actually need `printf`
  changes; `strftime` from 168 already produces its output.
...
- **`%f` / `%e` / `%g` in printf** — float specifiers. Blocked
  on chapter 171 lifting `-mgeneral-regs-only`. The formatter
...
Chapter 171 — FP/SIMD at EL0. Kernel change: set
