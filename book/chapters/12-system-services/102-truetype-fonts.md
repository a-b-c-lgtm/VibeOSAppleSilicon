# Chapter 102 -- TrueType fonts in the kernel

For 41 chapters the screen has been painted with the same VGA
8x16 bitmap font from chapter 46. It was a great call: a 256-byte
...
* [Chapter 46 -- virtio-gpu and the framebuffer](../05-devices/46-virtio-gpu-framebuffer.md)
...
* [Chapter 48 -- the window manager](../06-gui/48-window-manager-and-gui-syscalls.md)
...
* [Chapter 27 -- argc/argv](../05-devices/27-argc-argv.md) (which
  also surfaces in [chapter 101](101-guard-pages.md)'s
  postscript). This chapter rediscovers the same "16 KiB of
  kernel stack is not enormous" lesson the hard way -- so the
  diagnostic trick from chapter 101 applies here too.
...
`objcopy -I binary` trick the ramfs uses (chapter 13):
...
```c
if (fb_init() == 0) {
    font_init_ttf();     /* Chapter 102: upgrade default font to TTF */
    paint_boot_banner();
    ...
}
```
...
```c
SYS_GUI_MEASURE_TEXT = 52,   /* Chapter 102 */
```
...
called out in the
[chapter 70 sidebar](../08-browser/70-block-and-inline-layout.md).
...
* **Userspace, not kernel.** The rasteriser is a self-contained
  ...
  [chapter 108b](../14-userspace-services/108b-userspace-font-server.md)
