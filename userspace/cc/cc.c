/*
 * userspace/cc/cc.c — chapter 121's one-chapter native C compiler.
 *
 ...
 *
 * Not supported (lands in chapter 123's GCC port):
 ...
 * chapter 118's /bin/as does not emit).
 *
 * Per `printf("STR\n");` we emit:
 *
 *     bl   .LSjmpN
...
/* ──────────────────────────────────────────────────────────
 * Chapter 123 additions: local variable table + expression
...
/* ──────────────────────────────────────────────────────────
 * Expression parser + codegen.  Grammar (chapter 123):
 *
...
static void parse_program(void)
{
    ...
    /* Emit prologue. */
    emit_str("/* generated by /bin/cc — chapter 123 */\n");
    emit_str(".text\n");
...
int main(int argc, char **argv)
{
     ...
     *    Chapter 131f swapped the toy chapter-119 /bin/ld for the
     ...
     *    userspace/linker_user.ld (chapter 122 contract). */
    char ld_args[512];
    n = 0;
