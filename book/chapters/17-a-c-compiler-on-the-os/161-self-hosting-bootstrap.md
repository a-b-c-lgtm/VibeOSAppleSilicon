# Chapter 161 — The self-hosting gap, and why we don't close it here
...
- `/bin/cc` is exercised against its largest input yet.  No
  code changes — chapter 159's compiler is the one being
  stressed.
- `/data` mount + mkosfs2 seeding (chapter 158 / 124
  contract) is exercised again with a larger source.
- The `printf-clobbers-x0..x18` boundary that has been
  implicit since chapter 150 is *empirically* validated
...
Next: chapter 162, a tiny `/bin/make` that drives `/bin/cc`
