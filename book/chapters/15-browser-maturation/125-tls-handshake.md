# Chapter 125 — In-guest TLS handshake

Chapter 124 got `libbearssl.a` into the build tree and proved
...
tls handshake: PASS chapter 125 end-to-end
```
...
attributable. Once the loopback handshake passes, chapter 126
adds the CA store and chapter 127 points the browser at a real
...
The chapter-125 client *doesn't validate the chain at all*.
It pins on the public key of the leaf cert. (Real chain
validation comes in 126, once we have a CA store and a real
...
from the archive entirely (chapter 124). The link-time
...
where chapter 123 — the kernel CSPRNG and the `SYS_GETRANDOM`
...
```
/bin/init  -+----- spawn /bin/httpd   80    (chapter 111)
...
chapter 107 — a single-threaded `for(;;) socket_accept()` —
...
`init.c` already spawns `/bin/httpd 80` (chapter 111) so that
the desktop comes up with a usable local web server. Chapter
125 adds the matching line for TLS:
...
The test script mirrors `scripts/test_tlstest.py` (chapter 124)
...
- **New application: `/bin/httpsd`** — in-guest TLS server.
  ...
  good TLS endpoint can use it. Useful to chapter 127
  (browser HTTPS) and 128 (end-to-end public HTTPS).
- **New libc surface: `tls_socket.{h,c}`** — heap-allocated
  ...
  chapter 127 when the browser learns the `https://` scheme,
...
- **`tlstest`** — gains a `--handshake HOST PORT` mode that
  ...
  stays as the 124 smoke test).
- **`scripts/test_tls_handshake.py`** — new regression script
  added to the sweep.
- **Out of scope until 126**: real CA store, chain
  ...
  Chapter 126 builds the validator we'll actually trust.

## Next: chapter 126 — root certificates and chain validation
...
whose chain ends at one of them. Chapter 126 brings that in:
...
opens the door to chapter 127 — pointing the browser at a
real `https://` URL — and chapter 128, an end-to-end public
HTTPS fetch.
