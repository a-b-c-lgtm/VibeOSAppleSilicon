# Chapter 133 — Step 1: the mount table and `vfs_resolve`

[Chapter 132](132-mount-table-and-vtable.md) laid out the plan:
...
pulling drivers across in chapters 134 through 137.
...
- **`cookie` exists from day one even though every kernel
  ...
  Today there is only one tmpfs instance, but Chapter 140's
...
- **The methods take `rel` (the path *after* the mount
  ...
  own prefix back off. OSFS-1 in chapter 136 will register
...
- **`MOUNT_MAX = 16`.** Today we have six mounts. The cap is
  ...
  refcounting) while leaving headroom for chapter 140 to
  mount a handful of userspace servers without a recompile.

- **No locking on the table.** Mounts are added at boot only
  in this chapter. When chapter 140 introduces `SYS_MOUNT` we
...
chapter 140 when a userspace fs sends the same path back over
...
- **No de-duplication.** Two mounts at the same prefix are
  ...
  chapter 140 will reject duplicates at the syscall layer.
...
- **Prefix strings are borrowed, not copied.** Every caller
  ...
  lifetime, so this is safe. If chapter 140 ever wants to
...
a working abstraction. Chapter 134 will be the first chapter
