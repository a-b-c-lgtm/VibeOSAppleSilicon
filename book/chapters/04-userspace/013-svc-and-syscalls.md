...
machinery — is its own chapter: [Chapter 14](014-elf-and-first-user-program.md).
...
isolation until [Chapter 13B](#deferred-per-process-page-tables)
...
in [chapter 11](../03-time-and-concurrency/011-preemption.md#the-eret-window-trap);
...
- **ESR_EL1.EC = 0x15** is the encoding for "SVC instruction
  ...
  we built this layer. The fix is to *kill the thread* (call
...
header-only on purpose: the early user binaries are too
...
That single block exercises every path:
...
Chapter 16). At that point each `user_thread_create` will build
...
Next: [Chapter 14 — ELF loading and the first user program](014-elf-and-first-user-program.md).
