...
to splice in [Chapter 106a](106a-httpd-tls-bridge.md)) and
...
- [Chapter 105 -- `/bin/httpd`](105-bin-httpd.md) -- the
  static-file server.
- [Chapter 106 -- TCP loopback](106-tcp-loopback.md) --
...
- [Chapter 106a -- httpd as forwarding proxy](106a-httpd-tls-bridge.md)
...
- [Chapter 9 -- init, spawn, wait](../02-userspace/9-init-spawn-wait.md)
...
`#define` change and broke immediately. The chapter-106b
...
chapter-106b test exercises this on every run; without the
fix it cannot succeed.

The lesson is the same one chapter 30 made about input
...
2. **No TLS in the browser, ever.** The browser is now
   ...
   Adding TLS (chapter 112) is a change to httpd: stand up
...
- **Spawn order matters.** httpd must be listening before
  ...
  browser. The book leaves polling for chapter 11X when
...
The architecture chapter 106b lands is the same one big
...
transport, daemon does. Our chapter-106b version is the
simplest honest instance of that pattern -- one defined
constant in browser.c, one running daemon on a fixed
local port.

## What chapter 106a unlocks that this chapter uses

The chapter 106a `is_local_path` dispatch table is what
...
11. Assert httpd logged "forward GET ..." (chapter-106a
...
```
browser plain mode
  -> 127.0.0.1:8080            (chapter 106 loopback)
  -> httpd serve_forward       (chapter 106a)
...
If chapter 106 is reverted, step 10 fails: the browser's
`connect(127.0.0.1)` times out.

If chapter 106a is reverted, step 11 fails: httpd 404s
the unknown path, browser renders the 404 page, marker
never appears.

If chapter 106b is reverted (or someone reinstates the
...
- **OSFS bump:** [`kernel/core/osfs.h`](../../../kernel/core/osfs.h)
  ...
  the kernel header.
- **Existing tests unchanged:** every chapter-105/106/106a
...
- [`scripts/test_browser_proxy.py`](../../../scripts/test_browser_proxy.py)
  (new) -- regression.
- [`book/INDEX.md`](../../INDEX.md) -- milestone-97 row
  flipped from Planned -> Done.

## What this unlocks

- **Chapter 106c** -- the end-to-end celebration. Browser
...
- **Chapter 112 (TLS in httpd)** -- when we add a real
...
1. **The "apps must use the OS features" rule pays for
   ...
   chapter 106b. They all surfaced from one new
...
all eight fixes. Chapter 106c picks up from here.
