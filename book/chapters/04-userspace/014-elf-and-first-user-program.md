...
[Chapter 13](013-svc-and-syscalls.md) gave us a syscall channel and
...
Step 4 we already wrote in Chapter 13 (`user_thread_create` and
...
- **`USER_CFLAGS`** uses `-Os` instead of `-O2`. User code is
  small and rarely on a hot loop here; saving bytes in
...
- **Single PT_LOAD segment, all permissions.** Producing a single
  RWX segment is correct here because we don't have
  per-process page tables yet; the loader just allocates pages
  and copies the bytes. Once Chapter 16 introduces per-process
...
in — fine here because we look each one up by symbol.
...
guaranteed-zero. No argv/envp setup either; that's a Chapter 16
...
- **Per-process page tables.** Every user process shares the
  ...
  pmem. Chapter 16 fixes this by giving each process its own L1
  table.
...
- **A real syscall surface.** Four calls (`write`, `exit`,
  `getpid`, `yield`) is not a usable userspace. Chapter 15
...
- **Loading from disk.** The user binary is *embedded* in the
  kernel image. Chapter 15's VFS makes it possible to load
...
this chapter is so small — under 600 lines of new code, end to
...
- The `userspace_demo` in `kernel_main` boots → loads → spawns
  ...
  path above in 30 lines of kernel C.

Next chapter: [Files, VFS, and a simple file system](015-files-and-vfs.md).
