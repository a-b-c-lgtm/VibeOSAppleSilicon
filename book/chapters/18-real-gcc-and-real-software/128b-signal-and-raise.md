...
Almost all the engineering already happened, in chapters 77
...
- **Upstream headers reference them by name.** Vendored code
  ...
  separate decision you can take later (likely chapter 130c
...
- **Chapter 132 (GCC port)**: `gcc/diagnostic.c`'s
  ...
  No shim needed.
- **Chapter 130b (Doom platform shim)**: Doom's `I_Error`
...
- **`sigsetjmp` / `siglongjmp`** — these save and restore the
  *signal mask* in addition to chapter 128a's register state.
...
- **Synthesising `SIGSEGV` / `SIGFPE` / `SIGILL` from
  ...
  thread on a synchronous exception (chapter 19's
...
[Chapter 128c](128c-ctype-assert-string.md) adds `<ctype.h>`,
`<assert.h>`, and the missing `string.h` entrypoints
(`memmem`, `strdup`, `strndup`, `strchrnul`). Pure header-only
additions, no kernel work. Then 128d (`<time.h>`), 128e
(`qsort` / `bsearch` / `strtol` / `getopt`), 128f (real
`printf` / `scanf` / `fprintf`), and the kernel landmark —
**chapter 129** turns FP/SIMD on at EL0, the single biggest
