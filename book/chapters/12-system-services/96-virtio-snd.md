...
since chapter 41, and virtio-net since chapter 52. There is one
last virtio device on QEMU's `virt` bus that we have left
unspoken for: virtio-sound. Chapter 96 brings up a minimum-
...
* **Chapter 11 (again) / 17** — `pmem_alloc_page()` and
...
* **Chapter 56-style polling-with-yield** — `tx_submit_and_wait`
...
alive. Chapter 95 (RTC) hit the same trap; chapter 96
...
| `book/chapters/12-system-services/96-virtio-snd.md` | this chapter            |
...
Chapter 97 introduces a PNG decoder so the wallpaper and icons
...
Pillow trap (see `/memories/python-host-tools-pillow.md`)
deserves its own discussion when we revisit it.
