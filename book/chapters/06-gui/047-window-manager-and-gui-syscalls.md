# Chapter 47 — An in-kernel window manager and seven GUI syscalls

Chapters 45 and 47 gave us pixels and keystrokes.  This chapter
...
- **The WM owns decorations.**  The 24 px title bar and 1 px
  ...
  chapter 45.  Apps paint into the content area only.
...
rule when we add the mouse is the same: any new input device
...
int main(int argc, char **argv) {
    ...
    gui_draw_text(win, 16, 24, "Hello from the WM!",
...
## What's deferred (the to-do list)
...
- **Multi-app composition**: stagger logic exists, but the
  taskbar in `paint_wallpaper` is just a stripe — a later
  chapter will replace it with `desktopd`.
...
**focus = topmost & input is exclusive**.  Everything we add
later slots into that model without breaking it.
