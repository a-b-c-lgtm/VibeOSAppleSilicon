# Chapter 190 — `sys/` headers without a hierarchical filesystem
...
> still green. Prerequisite: chapter 189 (the 24 libc + 16
...
5. Verify the four chapter-187–189 tests stay green so
...
Chapter 189 shipped 24 user-facing libc headers + 16 GCC
...
The reason 189 skipped them: OSFS-1's root directory is
...
Option (2) is what 190 ships. It's about a hundred lines of
...
* **cpp**, when asked to expand `#include <sys/stat.h>`,
  ...
  gccw shim from chapter 187 injects `-isystem /bin`. So
...
4. **Stage the headers through a small rewriter at build
   ...
   (chapter 189 already proved `<stdio.h>` works).
...
24 entries are the chapter-189 set.
...
The file the test stats is one already shipped in 189 —
...
that 189's text-file shipping is reliable.
...
[chapter 190] in-guest #include <sys/stat.h>
...
Disk grew from 155 files (post-189) to 163 (post-190):
...
the pieces exist in the userfs implementation (chapter 132+)
...
unpack it into `/data/build/`. That's chapter 191:
`/bin/tar`. Once that ships, chapter 192 audits whether
the in-guest `make` (from chapter 162) handles the
DoomGeneric Makefile's pattern rules + automatic
variables; chapter 193 is the actual rebuild; chapter
194 is the smoke test where the rebuilt-in-guest Doom
plays the demo.
