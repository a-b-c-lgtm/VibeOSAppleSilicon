# Chapter 154 — An AArch64 assembler: /bin/as
...
> chapter 153 (stat / fcntl / dirent) and chapter 150
> (stdio).
...
[Chapter 157](157-bin-cc.md), or a future GCC port —
...
3. Emit a real ELF64-LSB relocatable object that the linker
   we write in [Chapter 155](155-bin-ld-linker.md) can consume.
...
plan. The linker in [Chapter 155](155-bin-ld-linker.md) relies
on this:
...
The linker in [Chapter 155](155-bin-ld-linker.md) is the one that
consumes these.
...
[Chapter 150](150-stdio.md). We'll reuse it
once more in [Chapter 155](155-bin-ld-linker.md) for `/bin/ld`.

## Applied to

There is no existing app for `/bin/as` to replace — this is a
new capability. The first consumer is the linker in
[Chapter 155](155-bin-ld-linker.md). The second is the tiny
compiler driver in [Chapter 157](157-bin-cc.md), which will
...
of 17** (chapter 153 baseline + `test_bin_as`).
...
[Chapter 155 — /bin/ld and /bin/ar](155-bin-ld-linker.md) takes
