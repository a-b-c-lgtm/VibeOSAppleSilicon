...
- Chapter 14 — SVC and the syscall ABI. The dispatcher in
  `svc_dispatch` is where we inject the tracer hook.
- Chapter 17 — `init`, `spawn`, `wait`. We need fork+exec
  semantics for `/bin/strace` to attach to a child.
- Chapter 99 — procfs. Chapter 100 borrows its
...
(chapter 99) exactly: open, render, slice; `tail` is the
...
bytes to stdout. This is the same trade-off chapter 99 made
...
freestanding code doesn't have. (See
`/memories/freestanding-c-memset-trap.md` and chapter 28
where this first bit us.) We dodge it the way the user
...
- Six creation paths in `thread.c`:
  ...
  `/* Chapter 100 — tracing is NOT inherited */` so future
...
This is the same eight-edit pattern chapter 99 introduced
...
be killed by a path that *does* free it. We've been bitten
by this before (chapter 75c on `fdt`), so the rule is now
...
`(not traced)\n` banner in that window — chapter 99 chose
...
of forgetting is hours of debugging. Chapter 99 paid this
cost for `FD_PROCFS`; chapter 100 paid it for
`struct strace_ring *`. The next per-thread thing will
pay it for whatever it is.

**Snapshot-on-open generalises.** Chapter 99 introduced
the pattern for procfs files. Chapter 100 reuses it for
