...
story analogous to chapter 79b's
[sys_spawn-skipped-fd-inheritance bug](chapters/09-process-model/79b-gui-term-real-processes.md).
...
8. **Retire the kernel surface.** Once steps 1-7 are green:
   ...
   - **Keep** the `SYS_SPAWN*` enum values reserved (do NOT
     ...
     chapters 9, 30, 31, 79b, etc., and a reader cross-
...
9. **Add a forward-pointer note** to chapter 9
   ...
   so book readers who hit chapter 17 first know the design
...
- **The reaper deadlock.** init's main loop has
  ...
  flow as today, only the child birth changed). See
  `/memories/kernel-thread-lifetime-reaper-trap.md`.
...
- **`exec` survives fd inheritance, including pty.** The
  pty bug from chapter 79b
  ([79b-gui-term-real-processes.md](chapters/09-process-model/79b-gui-term-real-processes.md))
...
- Chapter 17 ("init-spawn-wait") gains a forward-pointer
  note, but stays as written — it's still historically true.
- Chapter 30/31 ("pipes" / "pipelines") may need a small
...
- Chapter 17 has the forward-pointer paragraph.
- Chapters 30/31 have the errata note (or a rewrite).
- This chapter (79c) is rewritten from PLAN to RETROSPECTIVE,
  same shape as chapter 79b: what we changed, what bug we
