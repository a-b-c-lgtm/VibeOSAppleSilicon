# Chapter 105 -- /bin/httpd: a static-file HTTP server

Chapter 104 gave us `socket_listen` and `socket_accept`. Chapter
...
happen from a real browser**. Chapter 106 will finish that loop
by having our in-tree browser fetch from this in-tree server.

## Prerequisites

- [Chapter 104 -- accept and server sockets](104-accept-and-server-sockets.md):
  the syscalls and libc wrappers `socket_listen` /
  `socket_accept`; the FD_SOCKET_LISTEN fd kind.
- [Chapter 56 -- httpget](../07-networking/56-sockets-httpget.md):
  the client-side counterpart. Our server has the same shape,
  just with the read/write directions swapped.
- [Chapter 12 -- the OSFS mount at /mnt](../10-filesystem/12-osfs-mount.md):
...
`fstat()` syscall yet (chapter 110+ territory). The
alternatives are:
...
(chapter 91 added futexes but not a thread-pool primitive),
fork-with-shared-sockets is explicitly **not** supported (see
chapter 104), and our kernel doesn't have `epoll` or `kqueue`.
So we serialise. For a workload that's "two browser tabs
hitting localhost" this is fine; for production it'd be a
disaster. Chapter 105 is the simplest correct version, not
the fastest.
...
- **It composes with chapter 99's procfs.** A future hack
...
in our libc by chapter 56. The new code budget is the request
...
last new piece -- `socket_accept` -- arrived in chapter 104.
Chapter 105 is just the glue program that lights them up
in sequence.
...
- `book/INDEX.md` -- milestone-94 row added.

No kernel changes. All of httpd's needs were already covered
by chapter 104's syscall surface plus the existing
...
- **Content-Length**. Needs `fstat()` (chapter 110+) or
...
- **TLS**. Real TLS needs either BearSSL or a host-side
  proxy. Chapter 112 will pick one.
...
- **Chapter 106 -- end-to-end loop**. With httpd serving
