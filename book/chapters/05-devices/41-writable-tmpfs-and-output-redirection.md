# Chapter 41 — Writable tmpfs and `>` output redirection

## Why tmpfs first

We have read-only OSFS-1 (chapter 21) and an in-memory ramfs (chapter 17),
but no writable filesystem.  Pipelines (chapters 39–40) gave us a way to
move bytes between processes, and `<` (chapter 37) lets a program read
...
`sys_write` already had a `FD_PIPE_W` branch from chapter 39.  We add
...
existing `<` parser (chapter 37): scan the command line, splice out the
...
| no       | yes      | no        | `spawn_redir(path, args, redir_in)` (chapter 37)     |
...
tmpfs, but symmetric with the pipe case from chapter 40), and more
...
- No `ls /tmp/` — the existing `ls` (chapter 29) walks ramfs and OSFS
...
- `kernel/core/main.c`: banner bumped to milestone 32.
