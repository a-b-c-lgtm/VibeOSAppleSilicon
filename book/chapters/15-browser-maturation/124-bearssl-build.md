# Chapter 124 — BearSSL builds for our userspace

> **Milestone in this chapter:** 124 — vendor BearSSL and prove
...
> digest. No TLS handshake yet — chapter 125 runs the first one.

Chapter 123 added entropy. The kernel can now hand out
...
from our build, because chapter 123 already provided
`SYS_GETRANDOM` and BearSSL is happy to use any callback for
seeding (chapter 125 wires that up).
 ...
 * Return 0 — "epoch" — and let chapter 126 plug in the real
...
- The `-not -path ...` in the `find` skips `sysrng.c`. If we
  ...
  we provide. (Chapter 125 will substitute a BearSSL
...
Related: the freestanding-C `= {0}` trap is the companion
lesson — implicit `memset` calls from `= {0}` struct
inits in freestanding C, which we side-step by zeroing
fields by hand.
...
Run that *before* you instrument the library. The lesson —
verify the expected vector against an independent source
before you assume your implementation is wrong — is one of
those habits that pays for itself the first time it saves
an afternoon of pointless instrumentation.
...
- `userspace/libc/cstring.c` — extern `mem*`, `strlen`,
  `time` stub.
- `userspace/tlstest/tlstest.c` — chapter 124 SHA-256 KAT.
...
- `scripts/_dbg_tlstest_verbose.py` — captures the full boot
  log plus tlstest serial output. Kept under `scripts/`
  alongside every other `_dbg_*` helper as a one-off
  reference probe.

**Existing apps modified**
- None yet. `tlstest` is purely a regression binary. The first
  real app to use BearSSL is the browser, in chapter 127.
...
- **Chapter 125 — `tls_socket_*` and the SSL engine.** Wire
...
- **Chapter 126 — root CA store and chain validation.**
  Ship a small set of CA certificates as a userspace constant
  blob, plumb `br_x509_minimal_set_time` from the new
  `SYS_GETTIMEOFDAY` (chapter 96 wall clock).
- **Chapter 127 — browser `https://`.** Teach the browser
...
- **Chapter 128 — end-to-end public HTTPS.** A regression
...
promise that motivated chapter 123: the browser can speak to
the real web.
