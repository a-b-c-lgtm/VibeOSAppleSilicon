# Chapter 92 — Userspace threads (clone-shaped)
...
thread. Chapter 72's `fork` lets a process duplicate itself
...
* **Chapter 90** — SMP. Sets the chapter 92 floor:
  ...
  CPU 0.
* **Chapter 91** — mmap. The userspace `thread_spawn`
  helper allocates the worker's stack via
  `mmap(MAP_PRIVATE | MAP_ANONYMOUS)`, which means
  chapter 92 doesn't need its own page-allocator path.

## Address-space refcounting

Pre-chapter 92, the AS was an exclusive resource: every
...
Refcount changes are atomic because chapter 90 lets
...
(token)` from chapter 29 (pipes). The wait token is
...
machine (chapter 90's invariant), masking IRQs is enough
...
* **Always wakes on unlock**, even if no one was
  ...
  bookkeeping is messy and the chapter 92 floor doesn't
...
unmap). The chapter 92 floor is "create N threads,
...
them). It joins the chapter 91 sweep at slot 26.

## Chapter 90 SMP interaction

The chapter 90 invariant says: user threads stay on
...
threads in chapter 92 — just interleaved execution.
...
* **FDs are not shared across clone siblings.** Each
  ...
  The chapter 92 test doesn't need shared fds; a real
...
* **No `fork` after `clone`.** The fork code clones
  ...
  chapter 92 test). Mixing both is undefined.
...
* A practical proof that the chapter 90 SMP work was
  enough: the AS refcount + the futex queue both
  needed atomic operations that the chapter 87/87
...
* `book/INDEX.md` — chapter 92 row marked Done

## Build & test

```
make all
python3 scripts/test_threads.py     # chapter 92 smoke
