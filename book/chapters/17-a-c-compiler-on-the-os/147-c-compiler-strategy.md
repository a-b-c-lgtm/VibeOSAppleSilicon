# Chapter 147 — A C compiler that runs on the OS
...
> this chapter.** The actual implementations land in chapters 148
...
Chapter 161 spells out the gap between `/bin/cc` and a
...
file end-to-end" can stop after chapter 160. Chapters
...
- **A real GCC port.** That is left for Part XVIII.
  Chapter 161 catalogues the language work `/bin/cc`
...
- **A TinyCC port.** Considered and rejected for the
  ...
  user-visible until it links cleanly. Chapter 157
...
- **`/bin/cc`** (chapters 157, 123): a usable shell
  loop `cc foo.c -o /tmp/foo && /tmp/foo` for tiny
  programs.
- **`/bin/make`** (chapter 162): one-rule Makefiles
  driving the toolchain.
- **Notepad → Build → run** (chapter 163): an in-OS
...
- **An honest baseline for Part XVIII.** When the book
  ...
  toolchain contract (chapter 158), the libc surface
  (chapters 148–153), and `/bin/as` + `/bin/ld`
  (chapters 154–155) are already standing.

## Prerequisites

- Chapter 72 — fork (so `make` and `cc` can spawn
  helper processes).
- Chapter 73 — exec.
- Chapter 91 — mmap (would be useful for a future
  compiler that wants to mmap source files; `/bin/cc`
  uses `read` instead).
- Chapter 96 — wall-clock time (Makefile timestamp
  rules, `__DATE__` / `__TIME__` macros for a future
  preprocessor).
- Chapter 102 — strace (debugging the toolchain pipeline
