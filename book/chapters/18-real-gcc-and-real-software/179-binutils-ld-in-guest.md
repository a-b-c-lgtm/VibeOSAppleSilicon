# Chapter 179 — `ld` in-guest: cross-building binutils' linker
...
> 178 configure baseline still green. Prerequisites:
> chapters 175 (binutils-osdev), 176 (wrapper), 177
> (link-mode wrapper), 178 (CONFIG_SITE cache +
...
1. Cross-configure five more binutils subdirs in order
   ...
   the chapter 178 `CONFIG_SITE` cache plus two new
...
5. Close the eight smaller libc gaps the cross-build
   ...
   `OSDEV_LIBC_NO_GETOPT` replacement for 178's
   `ELIDE_CODE` patch).
...
Chapter 178 shipped one cross-built `libiberty.a`. It
...
The first three are gap-closing work in the chapter 178
...
**Stage 1: configure five more subdirs the same way 178
...
```python
"--disable-binutils",  # gas + ar + objdump etc. — 180
...
`time.h`, plot/graph TUI bits). 180's problem.
...
addition). The chapter 178 cache file marks
...
3. **Provide strong external symbols in one `.o` already
   ...
   (chapter 172) can carry the rest.
...
call it). Chapter 172's `strdup` originally lived on
malloc.h's heap; chapter 179 moves it to cstring.o's
...
13 strong-extern symbols ship in the chapter 179 block:
...
from chapter 172; the chapter 179 block only adds the
missing extras vendor archives reference.

`aarch64-elf-nm cstring.o` after the chapter 179 block
shows global `T` entries for all 13 names plus the
chapter-172 memcpy/memmove/memset/strlen — matching
...
`__cxa_finalize` (chapter 156) and `environ`
(chapter 151) are emitted by their respective
...
already declared in `crt0.S` (chapter 156 ships it as
...
172 (Doom's shim used the same trick). Both files
look like:
...
## The libiberty `getopt.c` patch — replacing 178's `ELIDE_CODE`

Chapter 178 patched `libiberty/getopt.c` to elide its
entire body under `__OSDEV_LIBC__` (using libiberty's
own GLIBC-elision mechanism). That worked for
`libiberty.a` compilation in 178 because nothing in
the canary subset linked against `_getopt_internal`.

`ld_new` does link against it: `libiberty/getopt1.c`
(`getopt_long`, *not* elided — see 178's "Why getopt1.c
...
elision does the job. Chapter 179 replaces hunk 5 of
...
+/* osdev libc (chapter 179): suppress userspace/libc/stdlib.h's
...
> Chapter 179: replaced the earlier 178 ELIDE_CODE
...
`-mcpu=cortex-a72` matches the QEMU target (chapter 16).
`-DNDEBUG` is new in 179: vendor TUs include
...
2. Wipes `build/binutils-build-guest-ld/` (separate from
   chapter 178's `build/binutils-build-guest/` so the
...
- **Chapter 180:** copy `build/binutils-build-guest-ld/ld/ld-new`
...
- **Chapter 181–185:** the GCC bring-up. Same shape as
  175–180 but for the compiler instead of the linker.
...
### Pitfall — reusing the chapter-178 build dir
...
configure flags (178 has `--disable-binutils` AND
`--disable-ld`; 179 drops the second). autoconf is
confused by a build dir with mismatched config.status;
worse, when 179 runs first and 178 runs second, 178's
sub-configure inherits 179's `--without-zstd` from the
...
chapter 172's Doom shim. **Rule:** never mark a symbol
...
weak *override* target (chapter 156's crt0 stub).
...
Chapter 180 carries `ld-new` across into the guest
