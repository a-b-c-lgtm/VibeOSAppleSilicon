...
do (chapter 4: we always use `EL1h`, i.e. `SP_EL1`). So at this
stage we have eight slots that can realistically fire:
...
- Slots 8–11: a fault while userspace is running (this becomes
  meaningful in chapter 14 when we ELF-load programs).
...
it spins in `wfe` forever. Chapter 13 will introduce the syscall
path and chapter 14 will introduce true kernel-recoverable faults
...
retires the early `stp/ldp`-on-Device-memory caveat by
