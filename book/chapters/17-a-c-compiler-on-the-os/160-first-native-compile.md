# Chapter 160 — The first native compile from disk

> "Disk-resident source, in-guest compiler, in-guest binary."

By chapter 159 `/bin/cc` could compile a string we typed at
...
1. Host: seed `/data/hello.c` into `build/data.img` with
   ...
   exact mechanism we documented in chapter 158 — it is the
...
Up to and including chapter 159, every `.c` source the OS
...
Chapter 160 cuts that dependency. The source lives on a
...
- `/bin/cc` itself is the app this chapter exercises. No
  code change to the compiler — chapter 159 already taught
...
| Compiler can only handle one `.c` per invocation | Chapter 162 (/bin/make) batches them |
| Test harness still pushes the source from host | Chapter 163 (notepad Build button) lets the *user* type the source in the guest |
...
1. The smallest "write a program and run it" loop is
   ...
   runs the binary. We had all four by chapter 157. What
   chapter 160 adds is making the *first* of those four
...
Next: chapter 161, the self-hosting bootstrap concept — how
