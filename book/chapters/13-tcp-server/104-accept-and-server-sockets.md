...
Chapter 103 taught the kernel how to **be** a TCP server:
...
up. Chapter 105 will replace the echo loop with an HTTP/1.0
parser to give us `/bin/httpd`; chapter 106 will close the loop
by having the in-tree browser fetch from the in-tree server.

## Prerequisites

- [Chapter 103 -- Passive open](103-passive-open-listen.md):
...
- [Chapter 56 -- Sockets and httpget](../07-networking/56-sockets-httpget.md):
...
`tcp_close` already handles both flavours: chapter 103 taught it
...
(milestone 91-ish territory: futex-style sleep on conn-state
...
1. **TCP has no callback hook into the scheduler.** Adding one
   ...
   plumbing -- chapter 105 territory or later.
...
self-test (chapter 103) uses port 8088 to verify passive open
...
in chapter 105+ will have proper exit conditions.
...
- **fork-inherited sockets**. Listening fds, like connected
  ...
  socket-table entry shared between processes -- chapter 110+
  territory in our roadmap.

## What this unlocks

- **`/bin/httpd`** (chapter 105). Drop in an HTTP/1.0 parser
  in place of `echo_one` and you've got a static-file server.
- **End-to-end loop testing** (chapter 106). Once `/bin/httpd`
