...
Up to milestone 5 the heap lived in a 16 MiB span the linker
...
For milestone 6 the only property we care about is `reg` on
...
freestanding equivalents for yet.  The whole point of milestone
6 is one focused task — read `/memory` — and writing it
...
trips up every first-time milestone-6 build: it does *not*
...
is making the CPU able to touch it.  Pre-milestone-6 the
...
Milestone 6 keeps L1[0] and L1[1] hardcoded — the kernel image,
...
Nothing else has to be excluded — the pre-milestone-5 boot
...
The heap allocator from chapter 13 does not care where its
...
Those user processes' actual memory — chapter 14's
ELF-loaded code, chapter 15's user heap — will come straight
...
during milestone 8's MMIO-mapping cleanup, when we start
caring about per-device address-space hygiene).

## Checkpoint

A milestone-6 build with the default 8 GiB target should
produce, in order:

* the milestone-5 banner output,
* an `[fdt] memory[0]` line confirming the DTB was found,
* a `[pmap] installed 0x8 x 1 GiB` line,
* a `[pmem] usable pages = …` summary,
* the milestone-3 heap smoke test,
* the milestone-5 preemption demo, and
* the milestone-2 heartbeat.

That cascade — DTB → mapping → frames → heap → threads →
preemption → ticks — is the full kernel-bootstrap pipeline
that everything from chapter 14 onward will build on top of.
