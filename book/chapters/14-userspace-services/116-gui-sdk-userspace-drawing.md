# Chapter 116 — Moving the GUI SDK into userspace
...
> to compositing damaged rects. Builds on chapter 114
> (mapped window buffers) and chapter 115 (userspace font
> server).

Chapter 114 gave userspace direct access to a window's pixel
buffer, but only the chapter-114 demo (`pixapp`) used it. Every
...
damage in place from 114 and a userspace font server in place
from 115, software rasterisation is just "write bytes into a
...
* `fontd` (chapter 115) became multi-client so the WM's
...
Three apps are deferred to chapter 117: `notepad`, `gui_term`,
and `browser`. All three are resizable, and chapter 114's
...
After 116 the kernel WM keeps:

* The compositor (`wm_compose_screen` and friends — z-order,
  blending, the cursor).
* The framebuffer.
* `wm_damage` (chapter 114): copy the user-VA pages back into
...
unmapped windows." If chapter 116 had broken either half, the
test would fail.
...
Chapter 114's `gui_window_fb` returned a struct with
...
implementation already knew the id (it's the input); 116 just
...
hand (`userspace/pixapp/pixapp.c`'s pre-116 init) leaves
...
and worth recording in detail. Before 116, `fontd` (chapter
115) was a single-threaded `accept` → `serve_conn` → `close`
...
The chapter 116 fix in `userspace/fontd/fontd.c`:
...
* The whole accept loop now spawns a per-client worker via
  ...
  the parent's file table (chapter 94).
...
(~8 MB) streamed in 16-row chunks. Before 116, each chunk
...
After 116 the wallpaper window is mapped, so the streaming
...
Chapter 114's reference app. The chapter-116 work here was
...
`gui_draw_text` × 2 per toast. After 116, those calls land
...
Chapter 117 will add a remap-on-resize protocol (the WM
...
`w->user_pages_n` is the chapter-114 field that's nonzero
...
* An app that never calls `gui_window_fb` (notify, plus
  anything pre-116 that hasn't been ported) keeps the
...
* [`scripts/test_notify_legacy.py`](../../../scripts/test_notify_legacy.py) —
  ...
  Chapter116`, screenshots, asserts the body BG and accent
  pixels are still where they should be. The implicit claim
  it codifies: chapter 116 did not delete or break the
...
changes. Pixel comparison against the pre-116 baseline is
...
trap. `draw.c` defines a tiny private `static memset` to
satisfy the implicit call.
...
chapter-114 primitive `gui_window_damage(id, ...)` is still
...
* **Apps unchanged on purpose:** `notify` (legacy-syscall
  exhibit).
* **Apps deferred to chapter 117:** `notepad`, `gui_term`,
  `browser` (all resizable; need 117's remap-on-resize).
...
* **Chapter 117 — resize coherence for mapped windows.** Now
...
* **A measurable reduction in EL0/EL1 traffic per repaint.**
  The taskbar clock tick was the worst pre-116 offender
...
116 that's the WM's only fontd client; every userspace app
has its own. If a future chapter lets the WM render decorations
client-side too (e.g. via a 117-style "decoration app" that
the WM merely composes), `wm_font.c` and the kernel's fontd
client could go away entirely. That is not chapter 116's
fight.
