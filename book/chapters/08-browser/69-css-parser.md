# Chapter 69 — A tiny CSS parser

[Chapter 68](68-dom-construction.md) gave us a tree of HTML
elements with attributes attached. By the end of M60,
...
- **`malloc` per node.** A typical stylesheet is a few hundred
  ...
  of one render. M17 gave us a 32 MiB user heap; we are not
...
- **`source_order` is captured per rule.** The cascade in real
  ...
  apply it yet — that's M62's job — but we record the
...
specificity of the *matching* chain, not the rule. M62 will
need this when it walks the rules in cascade order. For now
...
test harness's puppet. It mirrors the M59 / M60 drivers:
...
5. **If a second argument is given**, it slurps the HTML
   file, runs the M59/M60 pipeline (`html_tok_init` →
   `dom_init` → `dom_build`), and walks the DOM depth-first.
   For each element node it loops over the rules and prints
   the indices of any that match. This is the M61 contribution
   to the eventual "computed style per node" of M62.
...
(same stack-discipline rule from M59). No shared state,
...
PASS: CSS parser + matcher milestone-61 — all checks green
```
...
feed M62's layout box generator a list of "here are the
declarations that apply to this element".

## What this enables

- **M62: Block and inline layout.** Now that we know which
  rules apply to which nodes, M62 can resolve the cascade
...
- **M63: `/bin/browser`.** End-to-end pipeline: fetch URL →
