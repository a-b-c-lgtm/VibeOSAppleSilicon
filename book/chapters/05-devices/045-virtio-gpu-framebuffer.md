# Chapter 45 — virtio-gpu: a framebuffer at native resolution

Up to chapter 44 the kernel only knew how to paint to a serial
...
from `virtio-blk` (chapter 19).  The probe loop in
...
userspace uses (chapter 47), and the smoke test in
...
text_draw_string(40, 60, "virtio-gpu",
                 LIGHT_BLUE, BLACK, 1);
...
same QMP `screendump` trick that chapters 46 and 48 build on:
...
- **Cursor sprite via `RESOURCE_CREATE_2D` + `MOVE_CURSOR`** —
  saved for when we add a mouse.
- **Hardware acceleration** — we never enable virgl; all
  composition is CPU-side in chapter 47's WM.
...
## Postscript: the bitmap font gave way to TrueType

For 41 chapters the 8×16 VGA font carried the entire GUI: the
banner above, the gui_term in chapter 49, notepad in chapter 50,
the clock in 56, the launcher and taskbar, and the browser's
text runs in chapters 69–70. By chapter 104 the price of looking
like a 1985 console outweighed the simplicity. Chapter 104
...
- **`text_draw_string` kept the same signature.** Internally it
  ...
  callers in the WM (chapter 47) didn't change.
...
- **What did change is *per-glyph width*.** The bitmap font was
  ...
  caret position or to centre a label, that's the legacy
...
- **One stack overflow story.** The first integration crashed at
  ...
  file-scope statics. Chapter 104 walks through the diagnosis
...
font path, jump to chapter 104 for the rasteriser and
chapter 115 for the (planned) move into userspace. If you're
