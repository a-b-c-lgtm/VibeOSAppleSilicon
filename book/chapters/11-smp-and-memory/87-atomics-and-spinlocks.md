# Chapter 87 — Atomics, barriers, and the audit of every shared global

Chapter 86 woke CPU 1. CPU 1 doesn't actually do anything yet —
...
chapter 89's scheduler runs threads on CPU 1, every plain
...
know exactly what chapter 89 will have to lock.
...
- Chapter 86 — second core via PSCI (atomics need two CPUs to
  be testable).
...
## What chapter 87 ships in `atomic.h`
...
(`secondary_main`, chapter 86). `isb_` belongs to the boot
...
|------------------|----------------------------|---------------------------|
| `kernel/core/thread.c` | runqueue + thread table | one big spinlock first; per-CPU + work-stealing later (chapter 89) |
...
| `kernel/core/heap.c`   | kheap free-list         | one spinlock around the whole allocator (chapter 89) |
| `kernel/core/pmem.c`   | `g_free_head`, `g_free_pages` | one spinlock around alloc/free (chapter 89) |
| `kernel/core/wm.c`     | window list + focus     | one spinlock (chapter 89; today only one thread mutates this) |
| `kernel/core/osfs2_cache.c` | LRU list + slab     | one spinlock per cache (chapter 89) |
| `kernel/core/tcp.c`    | socket table + TCB list | one spinlock (chapter 88 or 89; IRQ-safe variant required) |
...
| `kernel/core/serial.c` | PL011 + `g_serial_lock` | **Done in chapter 86** |
...
For chapter 87 these are all left as-is because (a) only one
CPU runs the network stack today, and (b) chapter 88's IPI
...
- **No ticket-spinlock upgrade.** The flat test-and-set
  `spinlock_t` in `arch/spinlock.h` (chapter 86) is unfair —
  ...
  pattern, that unfairness is unmeasurable. If chapter 89
...
used `dsb sy` in `secondary_main`'s READY publish (chapter 86)
because it was easier to reason about during bring-up; the
correct version is `dmb_ishst()`, and chapter 88 will switch
...
- **Chapter 88 — IPIs via GICv3 SGIs.** Now that we have
...
- **Chapter 89 — SMP scheduler.** The category-4 locks finally
