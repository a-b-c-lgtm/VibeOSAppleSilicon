...
layer's end-to-end behaviour:
    ...
    T1 -- fopen("/mnt/poem.txt","r") + fread loop hits EOF cleanly.
    T2 -- fopen("/data/stdio_out","w") + fwrite of a known pattern;
          fclose flushes buffered bytes to the fd.
    T3 -- re-open + fread back the pattern, byte-equality.
    T4 -- fseek(SEEK_SET=100) + ftell(==100) + fread returns the
          expected pattern bytes at that offset.
    T5 -- fseek(SEEK_END=0) + ftell == filesize; rewind + ftell==0.
    T6 -- fopen("/no/such/...","r") returns NULL with errno=ENOENT.
    T7 -- fputc / fgetc round trip through /data/stdio_chars.
    T8 -- fprintf(stderr, ...) reaches the serial output.
...
SERIAL_SOCK = "/tmp/osdev-libc-stdio.sock"
...
PASS_RE = re.compile(rb"\[stdiotest\]\s+(T\d)\s+PASS")
FAIL_RE = re.compile(rb"\[stdiotest\]\s+(T\d)\s+FAIL")
...
def main():
    ...
    try:
        ...
        out = send_cmd(s, "/bin/stdiotest", timeout=20.0)

        passes = {m.group(1).decode() for m in PASS_RE.finditer(out)}
        fails  = {m.group(1).decode() for m in FAIL_RE.finditer(out)}

        for t, label in [
            ("T1", "fread /mnt/poem.txt EOF + checksum"),
            ("T2", "fwrite 30x200B to /data/stdio_out + fclose flush"),
            ("T3", "fread back /data/stdio_out byte-equality"),
            ("T4", "fseek+ftell SEEK_SET=100 lands on pattern byte"),
            ("T5", "fseek SEEK_END returns filesize; rewind returns 0"),
            ("T6", "fopen missing -> NULL with errno=ENOENT"),
            ("T7", "fputc/fgetc round trip through /data/stdio_chars"),
            ("T8", "fprintf(stderr, ...) reaches the serial output"),
        ]:
            expect(t in passes and t not in fails, label)

        expect(b"[stdiotest] ALL PASS" in out,
