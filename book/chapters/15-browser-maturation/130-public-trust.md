# Chapter 130 — Direct outbound HTTPS: real public CAs

## What we built before this chapter

By the end of chapter 129 the in-guest TLS stack does, on
...
The BearSSL sample roots stay in (so the chapter 127/e/f tests
...
A debug harness — named with the `_dbg_*` prefix used
throughout the tree for one-off probes that stay out of
the regression sweep — that boots
...
* **SNI**. `tls_socket_connect(t, ip, port, sni)` (chapter 125)
...
* **Outbound TCP to public IPs:port 443**. The chapter-39 socket
  ...
  TCP code (chapter 38 / 55) doesn't care whether the destination
  is loopback or routable. SLIRP NATs it out the host's TCP
  stack.
* **The validator**. Chapter 129's recursive chain walk is the
...
3. **`TLS_MAX_ANCHORS` overflow is silent in the bundle parser**
   — chapter 129 noted this and bumped to 32; chapter 130
...
* `userspace/browser/browser.c`: continues to use the same
  `br_conn_open` path from chapter 128/f. `BR_CA_BUNDLE_MAX`
...
* `scripts/test_tls_pem_bundle.py` (chapter 129, now loosened):
