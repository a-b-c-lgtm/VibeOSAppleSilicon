# Chapter 63 — TCP and a kernel-side socket API

[Chapter 62](62-icmp-udp-dhcp.md) ended with a stack that could
...
- **LISTEN / ACCEPT.** Server-side TCP. Milestone 56.
- **A userspace socket API.** Right now `tcp_*` is callable
  ...
  step toward an HTTP-fetching userspace tool. Same milestone
  bracket as LISTEN.
...
## Postscript (milestone 64): the receive window in practice

The "4 KiB ring" of this chapter survived through M55-M62
...
browser milestones rendered only a hand-built test page around
