...
1. Write [scripts/aarch64-osdev-cc.in](../../../scripts/aarch64-osdev-cc.in)
   ...
   the chapter 131a toolchain and the OSdev libc.
...
**`-B build/toolchain/bin/`** adds the chapter 131a
...
libc (chapters 116–128) ahead of newlib in the
...
- Bypassing newlib for everything else is the **point**
  of chapter 131c — let gas's autoconf surface
...
both Homebrew's binutils and the chapter 131a binutils
...
132's real `aarch64-osdev-gcc` will be configured with
...
and exercises the chapter 131a build path. Half B of the
...
chapter 131a `aarch64-osdev-strip`, byte-compare to the
...
for this input class, which is the contract chapter 131c
needs.
...
## What 131c will need from this

Chapter 131c needs to cross-build real binutils gas — a
...
Two issues will surface in 131c that this wrapper
...
Both are correctly chapter 131c work, not 131b.
...
- No existing apps changed — the wrapper is
  forward-compatible scaffolding for 131c/d/132.
...
Chapter 131c uses this wrapper to cross-build real
