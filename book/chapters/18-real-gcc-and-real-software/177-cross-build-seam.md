# Chapter 177 — Cross-build seam: link-mode wrapper, first libc gaps

> **Milestone in this chapter:** extend the chapter-176 wrapper
...
> that chapter 178 will close. Prerequisite: chapter 176
...
1. Teach the chapter 176 wrapper a **link mode** that
...
3. Run `make -k all-libiberty` to flush out every remaining
   ...
   work-streams chapter 178 will eat.
...
"the host can configure binutils" (175) and "the target
compiler stand-in exists" (176) into "the cross-build is
...
Chapter 176's wrapper was a transparent passthrough that
...
So the **first** thing chapter 177 has to do is give
...
flags so the Makefile's identical flags (chapter 49+) still
...
`stdint-gcc.h` (the freestanding header gcc ships under its
own `include/` directory — e.g.
`lib/gcc/aarch64-elf/<ver>/include/stdint-gcc.h` in a host
gcc install).
...
clang reason chapter 175 hit (the bundled zlib has K&R
...
(chapter 166), but `<stdlib.h>` doesn't include
...
since chapter 18, but the unbounded variants were never
...
finds them. That's chapter 178's main body. It also
solves the multi-TU state problem noted in chapter 176
...
- `mktemp` is `mkstemp` minus the file open — generate
  ...
  `<stdlib.h>` somewhere? (check during 178).
...
- `ldexp`/`frexp` are `<math.h>` and need FP at EL0
  (chapter 171). 178 papers over them with stubs that
...
chapter 153) was shaped for the OSdev filesystem layout
...
declares it as `int open(const char *, int)`. 178 moves
...
| B (missing functions) | 10 | Easy — straight wrappers, except ldexp/frexp which wait for chapter 171 |
...
3. **Class B function stubs.** Wrap the underlying
   ...
   comment pointing at chapter 171.
...
- [scripts/test_guest_configure.py](../../../scripts/test_guest_configure.py)
  — new host smoke test (kept under `scripts/` per the
  repo's debug-scripts convention; the leading underscore
  is reserved for one-off `_dbg_*` / `_snoop_*` helpers,
  and this one runs as a regression check).
- `build/binutils-build-guest/` is auto-created on demand
  by the smoke test; covered by the existing `build/*`
  `.gitignore` rule.

No existing apps changed — chapter 177 is host-tool
plumbing. The apps-must-use-features rule applies when
chapter 178's libc.a refactor lands
(every app gets re-linked) and when chapter 179 replaces
...
- New: [scripts/test_guest_configure.py](../../../scripts/test_guest_configure.py).
  ...
  3. five known-good libiberty `.o` files build cleanly:
     ...
     `cplus-dem.o` (exercises chapter 177's `sprintf`),
     `regex.o` (exercises chapter 177's
     `abort`-via-`stdlib.h` cascade).
- Unchanged: chapter 176's
  [scripts/test_aarch64_osdev_cc.py](../../../scripts/test_aarch64_osdev_cc.py)
...
Chapter 178 eats the gap catalog above: refactor the libc
