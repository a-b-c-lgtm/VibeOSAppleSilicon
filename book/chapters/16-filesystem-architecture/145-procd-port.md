# Chapter 145 — Step 5: porting `procfs` to userspace

Chapter 144 ported `clipboardd` to userfs and showed that a
...
mechanical once 143 has landed). The interesting bits are
the bugs that the port surfaced — one in `libfs` (a wrong
listdir type convention), one in `kernel/core/pipe.c` (a
signal-vs-data race that has been latent since chapter 77)
...
walk `g_threads[]` directly. So 145 adds three syscalls
...
parallel enums since chapter 13 because freestanding C
...
The chapter-143 writeup left the `on_listdir` type
...
chapter 77 added SIGCHLD. The chapter-99 procfs never
triggered it; the chapter-145 port surfaced it
...
`top` in chapter 148), we'll reassess: if the struct
...
Chapter 146 closes the loop on both: it adds per-request
