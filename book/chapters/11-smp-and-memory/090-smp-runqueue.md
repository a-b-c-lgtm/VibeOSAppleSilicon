# Chapter 90 — SMP runqueue: per-CPU current, per-CPU runq, no migration
...
What chapter 90 does **not** do, deliberately, is migrate
...
- Chapter 10 — uniprocessor scheduler, runqueue, `cswitch_to`.
- Chapter 87 — `struct cpu`, TPIDR_EL1, secondary boot.
- Chapter 88 — atomics (the per-CPU `g_thread_count`, RESCHED
  ack counter, etc).
- Chapter 89 — IPI vocabulary (we'll add a third vector,
  `IPI_RESCHED`, here).
...
of those needs to become per-CPU. Chapter 88's audit table
flagged them as Category 2 ("per-CPU"), and chapter 90 acts
on that flag.
...
drive the runqueue. The reflex is wrong in chapter 90.
...
fired on first interactive boot of chapter 90. Two interacting
bugs:
...
Both fixes are also captured in chapter 90's repo memory and
...
The user-visible change in chapter 90 is one new function:
...
implementation bug in chapter 90, but it was instructive
...
For chapter 90 we just fix the one offender (the
sleeper-wake walk). For chapter 91+ this becomes a general
...
- **No CPU 1 timer.** No preemption on CPU 1. CPU-1
  ...
  exit.
- **No per-CPU heap or pmem locks.** Chapter 88's audit
...
chapter 90 with smoke tests passing and only discover the
