# Chapter 197 — Real key-up events

> **Milestone in this chapter:** retire the 250 ms timed-release
> input shim from chapter 173 and replace it with real key-up
...
> chapter-173 timer shim deleted. Prerequisites: chapter 29
> (virtio-input keyboard), chapter 117 (kernel WM focus
> model), chapter 173 (the stopgap whose retirement this
> chapter is about).
...
Two reasons. The bookkeeping reason: chapter 173 shipped
...
Re-read the chapter-173 postscript and the asymmetry
jumps out. `gamekeydown[k]` was being cleared *too
quickly* by the original chapter-172 shim (synthetic
release back-to-back with press, before `G_BuildTiccmd`
could see the held state); the 173 fix was to clear it
*too slowly* by 250 ms. The lower-bound argument in 173
...
key lift. That's what playing Doom over the chapter-173
shim feels like.
...
checked manually with the chapter-174 nine-step
...
`type_key`-style QMP injection (chapter 174) doesn't
...
- **`/bin/doom`** is the canonical consumer of
  `GUI_EVENT_KEY_UP`. The chapter-173 timed-release
...
answer is `/bin/doom` (chapter 172 / 174). The shim
...
196 was the closing of the "rebuild Doom in-guest"
arc, and this chapter is the closing of the deferred
input issue that chapter 174 flagged as "not 174's
