# Chapter 182 — GMP, MPFR, MPC as in-tree prerequisites
...
> chapter 181 (the patched gcc source tree must exist before
...
- It also downloads ISL by default (~3 MiB) for Graphite
  ...
  optimization.
- Chapter 183's "does it build" smoke test is the
...
reason: `make clean-gcc-src` (chapter 181) blows away
...
build happens in chapter 183 via gcc's top-level
...
183 will get all of that for free.
...
- Unchanged: `scripts/sweep.sh`. Still 61/61 from chapter
  180 — no guest-side code changed.

## What this unlocks

- New host build target: `make gcc-osdev-prereqs` (Makefile,
  in the chapter-182 block after the chapter-181 `clean-gcc-src`
  rule).
...
- Wiring: `$(GCC_PREREQS_MARKER)` added as a prereq of
  ...
  chapter-181 `$(GCC_MARKER)`. A fresh clone running
...
- **183** — the actual cross-gcc build. Sibling build dir
  ...
  way chapter 179 cranked binutils.
- **183 follow-up** — extend
  ...
  the OSdev libc + crt0 + linker script. (181 deliberately
