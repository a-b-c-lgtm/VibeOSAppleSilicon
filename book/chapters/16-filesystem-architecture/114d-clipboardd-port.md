# Chapter 114d — Step 4: porting `clipboardd` to userfs

Chapter 108 built a userspace clipboard daemon
...
That's the whole point of chapter 114. Userfs gave us a way
...
daemon we port (procd, [chapter 114e](114e-procd.md)) is
...
shell. With chapter 114 in place, `echo foo > /clipboard/text`
...
Chapter 108's regression had to drive eight assertions
...
The bug was latent: chapter 114c's `echofs` test
...
load-bearing for every future userfs daemon. Chapter 114e
will exercise the same path under heavier load (procd's
`/proc/<pid>/status` files are kilobytes apiece, where
clipboardd's are single-line reads); chapter 114f will add
