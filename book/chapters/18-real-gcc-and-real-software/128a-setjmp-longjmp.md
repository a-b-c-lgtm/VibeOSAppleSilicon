...
Part XVIII's [chapter 128 plan](128-plan-real-gcc-and-doom.md)
...
- **It's the smallest unit of real ABI engineering.** AArch64's
  ...
  there is no in-between. Good warm-up for chapter 129's FP-state
  context-switch work.
...
1. [userspace/libc/setjmp.h](../../../userspace/libc/setjmp.h) —
   ...
   documentation pins down the buffer layout so chapter 129
...
instruction that does run. Chapter 129 is going to flip both —
...
```
   ...
   80..95  x29, x30
       96  sp
  104..   d8..d15   ── reserved for chapter 129
      168  pad      ── keeps size a multiple of 16
```

When chapter 129 lands it will add eight `stp` pairs in the
...
that compiled and ran before chapter 129 didn't have FP regs
...
- **The libc gains `setjmp` / `longjmp`.** Nothing in-tree
  ...
  chapter is the foundation, and chapters 128b–f / 131–133
  are the consumers.
- **The build** picks up `setjmp.S` automatically via the
  existing `userspace/%.S` pattern rule — no new wiring.
- **The chapter-132 GCC port** can be cross-built without a
...
- **The chapter-130c cross-built Doom port** sees a libc
...
[Chapter 128b](128b-signal-and-raise.md) adds `signal()` /
`raise()` wrappers on top of chapter 77's `sigaction`, plus
...
(chapters 128c–f), then the kernel landmark — **chapter 129**
