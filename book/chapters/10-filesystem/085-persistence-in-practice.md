# Chapter 85 — Save As: dialogs, libraries, and the first widget toolkit

OSFS-2 (chapter 82) gave us a writable disk.  Chapter 84 made
those writes crash-consistent.  Chapter 50 gave us a notepad.
...
- `/bin/paint` (chapter 69's BMP exporter)
...
move to chapter 86 (split off because they have nothing to
...
- Chapter 50 — notepad's original single-path save logic.
- Chapter 82 — OSFS-2's flat namespace (which is why the
  dialog rejects `/` in filenames).
- Chapter 84 — the journal that makes the saved file
  durable across crashes.
- Chapter 86 — (next) shell history and browser cookies on
  `/data/`.
