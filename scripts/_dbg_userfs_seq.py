#!/usr/bin/env python3
"""Trace exact test_userfs_echo.py sequence with verbose serial dump."""
import os, socket, subprocess, sys, time, select

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SERIAL_SOCK = '/tmp/osdev-userfs-seq.sock'
...
try:
    ...
    for cmd in [b'/bin/echofs &\n',
                b'/bin/cat /echo/hello\n',
                b'/bin/echo userfs-write-test > /echo/buf\n',
                b'/bin/cat /echo/buf\n',
                b'/bin/ls /echo\n']:
        print('=== sending: ' + cmd.decode().strip() + ' ===')
        s.sendall(cmd)
        time.sleep(0.3)
        print(drain(s, 6.0).decode('utf-8','replace'))
finally:
    s.close()
    q.kill()
    try: os.unlink(SERIAL_SOCK)
