...
5. Add `scripts/test_gcc_stdio.py` (cpp expansion, ELF
   ...
   the chapter-132g and chapter-132h tests still green.

---

## Why now

Chapter 132h proved that `/bin/gcc bf.c -o /tmp/bf2` works
...
The reason chapter 132h had to cheat is laid out in that
...
So chapter 132h punted to "freestanding-by-design" and left
...
```c
/* Bump chain: 32 (pre-M60) -> 64 (M60) -> 128 (chapter 106b)
 * -> 256 (chapter 132i, libc + GCC headers on /bin). */
...
132g (libraries, programs, startfiles). It was not enough
for cpp.
...
Half of this lesson was learned in chapter 132g when `-L`
...
```c
/* Chapter 132g: -B/bin/ tells xgcc to look in /bin for
 * crt0.o, libosdevc.a, and the linker script.  Chapter
 * 132i: -isystem /bin extends cpp's `<>` search path to
...
These are the headers chapters 116–128 already maintain
...
(132j). Until then, in-guest code that wants `unistd.h`
...
**Fix:** Follow the chapter 132h pattern — ship the source
...
in-guest shell. Always ship via mkosfs. The chapter 132h
...
stale-image bugs in chapters 124 and 128.
...
```
[chapter 132i] /bin/gcc resolves #include <stdio.h>
...
Regression: `test_gcc_hello.py` (chapter 132g) still PASS
10/10. `test_gcc_bf.py` (chapter 132h) still PASS 6/6.
...
- **`#include <unistd.h>`** — the header itself is on disk
  ...
  yet. Chapter 132j adds a single-level subdir to OSFS-1
  and then ships `sys/`.
...
- That `make` finds and respects the headers in /bin
  (chapter 126 ported a make; integration left to chapter
  133).
...
- **Existing tests still green:**
  - `test_gcc_hello.py` (chapter 132g) — PASS 10/10
  - `test_gcc_bf.py` (chapter 132h) — PASS 6/6

The "ship the source alongside the binary" pattern from
chapter 132h now extends to "ship the headers alongside
...
- **Chapter 132j** builds sys/-aware OSFS-1 and ships
...
- **Chapter 130a** (doom port) gets a clearer path:
