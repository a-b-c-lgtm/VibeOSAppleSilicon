# Chapter 93 — Real SMP scheduling: per-CPU timers, locked sleeper walks, CLONE_CPU

> **Milestone in this chapter:** close the three problems
> chapter 90 deferred — the secondary CPU's timer, the
...
Chapter 90 brought up the second core and gave us per-CPU
...
specific CPU. Chapter 92 then *relied* on every user
...
Chapter 93 lifts all of these limitations at once. We
...
target thread's home CPU, swap chapter 92's IRQ-disable
...
* **`prev == next` fast-path in `yield()`.** A subtle
  case unique to chapter 93: a cross-CPU wake can shove
...
* **Chapter 87** — PSCI + secondary boot. We now reuse
  `secondary_main` to enable per-CPU timer state;
  chapter 87 set up the boot sequence into it.
* **Chapter 88** — atomics. `g_ticks` becomes atomic via
  `atomic_add_return64`; the address-space refcount
  reused by clone2 keeps using `atomic_add_return32`.
* **Chapter 89** — IPIs. `runq_push_remote` already
  sends `IPI_RESCHED`; chapter 93 just calls it more
  often, from places that previously assumed a single
  CPU.
* **Chapter 90** — SMP runqueue. Lifts the floor that
...
its own copy. The chapter 90 secondary skipped that step
...
void secondary_main(void) {
    ...
    /* Chapter 93 — secondaries now run user code, so they
...
`timer_init_per_cpu` already existed from chapter 90
...
Chapter 92 added futex_wait/wake, both of which walk the
list. Chapter 93 puts user threads on CPU 1 too, so two
...
Chapter 93 hits a case `yield()` had never seen before:
...
Pre-chapter 93 this couldn't happen because cross-CPU
wakes didn't exist. Chapter 93 ships the fast-path:
...
Chapter 92's futex_wait used IRQ-disable to fence the
"check predicate, mark BLOCKED, yield" sequence:

```c
/* chapter 92 — single-CPU floor */
...
This worked because chapter 90's invariant pinned every
...
Chapter 93 puts user threads on CPU 1 too. A CPU-1
...
g_all_head, and chapter 93 wraps that walk in the
same `g_all_lock`.

## `SYS_CLONE2` and `SYS_GETCPU`

The new syscalls are at slots 75 and 76, right after
chapter 92's SYS_FUTEX_WAKE = 74.
...
enum {
    ...
    SYS_FUTEX_WAIT  = 73,
    SYS_FUTEX_WAKE  = 74,
    /* Chapter 93 — explicit CPU placement + getcpu. */
    SYS_CLONE2      = 75,
    SYS_GETCPU      = 76,
};
```
...
the chapter 92 sibling: it sets `t->home_cpu = cpu_id`
...
for the cpu_id arg — chapter 92 only ever needed up to
...
harness would fail to match either. Chapter 93's
...
* **Threads do not migrate.** Once `home_cpu` is set
  ...
  balance is a separate chapter; earlier chapters punted on it
  too.)
...
* **`futex_wake` ignores n.** Still treated as "wake
  all" — chapter 92's floor still applies. Bounded
...
* **No kernel-side preemption disable.** A long
  ...
  preempt counters, and chapter 93 doesn't add them.
...
1. **Live TLB invalidation** — moving a thread off
   ...
   from chapter 89, but the bookkeeping (tracking
...
A "good enough" first cut is exactly what chapter 93
...
* `book/INDEX.md` — chapter 93 row added under Part XI

## Build & test

```
make all
python3 scripts/test_threads_smp.py     # chapter 93 smoke

# 27-test regression sweep (chapter 92's 26 + new SMP test).
...
chapter 92 baseline before any chapter 93 work — they
are tracked separately and not chapter 93 regressions.)
