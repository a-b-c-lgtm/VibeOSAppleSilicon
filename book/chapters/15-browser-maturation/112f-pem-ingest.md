# Chapter 112f — PEM ingest and the recursive chain walk

Chapter 112e shipped a multi-anchor trust store. The two
...
Chapter 112f takes two small steps that unlock that walk:
...
- `Makefile` — the `assets/osfs/ca.bundle` rule now points at
  ...
  signed the leaves in chapter 112e's anchors).
...
- `scripts/test_tls_pem_bundle.py` — chapter-112f regression.
  ...
  with the chapter-112f bundle proves the validator walked
...
chapter-112e rationale carries over unchanged.

## What "recursive chain walk" actually means here

Before chapter 112f the validator's job was simple:
...
implementation; chapter 112c already wired it in correctly,
but 112e's "trust the intermediate" shortcut hid the recursive
case. 112f unhides it.
...
If a future bundle ingests more than 8 anchors, chapter-112e's
...
debugging session. Chapter 112f raises the cap to 32 and
...
- `scripts/test_tls_pem_bundle.py` (new) — the chapter-112f
...
- DNS for `example.com` through SLIRP (already works in
  `scripts/test_dns.py` since chapter 38).
- Outbound TCP to port 443 through SLIRP (chapter 36's
  `virtio-net` setup already covers this).
- A regression policy decision: live-internet tests are
  brittle for CI, so chapter 112g will likely ship the
