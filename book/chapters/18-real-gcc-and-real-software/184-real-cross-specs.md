# Chapter 184 — Real cross-compiler specs: retiring the wrapper

> **Milestone in this chapter:** bake everything the link needs
> into gcc's own target header and sysroot so the chapter-176
...
> gcc's standard sysroot. The chapter-176 shell wrapper is
> kept as a reference artefact but is no longer invoked.
> Prerequisites: chapters 175 (binutils-osdev), 176
> (wrapper), 181–183 (gcc target + prereqs + cross-build).
...
5. Stop using the chapter-176 `aarch64-osdev-cc`
...
Chapter 183 installed `aarch64-osdev-gcc` with the
...
around by chapter 176's `aarch64-osdev-cc` shell wrapper.
...
named spec is overridable per-target.  Chapter 183 built
...
Chapter 176 papered over all five with a 200-line
...
ever benefited. Chapter 184 teaches the *real* compiler
...
The chapter-178 libc-gap work added `#ifdef __OSDEV_LIBC__`
guards inside vendored gnulib / libiberty sources
(`getopt.c`, `obstack.c`, etc.) to swap in the OSdev
header-only shims.  Chapter 176 passed `-D__OSDEV_LIBC__`
...
with `--with-sysroot=…` — done in chapter 184's Makefile
...
mirrored inside the guest in chapter 185 — when xgcc is
...
The chapter-184 block (after `clean-xgcc`):
...
*unprefixed* tool names there.  The chapter-175 binutils
...
- New configure flag: `--with-sysroot=$(TOOLCHAIN_PREFIX)/aarch64-osdev`
  in [Makefile](../../../Makefile) (chapter-183 block).
...
- New Makefile targets in the chapter-184 block
...
- Patch file [vendor/gcc-aarch64-osdev.patch](../../../vendor/gcc-aarch64-osdev.patch)
  ...
  `patch -p1` reproduces the chapter-184 compiler).
- No existing apps changed — host-only chapter.  Chapter
  185 ships this into the guest and lets the in-guest
  shell drive it.
...
- Existing: `scripts/test_xgcc_build.py` (chapter 183) —
  still PASS.
...
- **185** — ship `aarch64-osdev-gcc` (plus binutils, plus
  the mirrored sysroot) into the guest disk image.
  In-guest test:
  ```
  /bin/gcc /tmp/hello.c -o /tmp/hello && /tmp/hello
  ```
  closes Phase 2 of the chapter 164 plan.
- **191+** — Phase 3: native-compile `doomgeneric` inside
