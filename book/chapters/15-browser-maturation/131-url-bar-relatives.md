# Chapter 131 — URL-bar UX after native TLS

## What was suddenly wrong after 130

Chapter 130 shipped two things: real public CA roots, and an
...
1. **Bare hostnames went through the proxy.**
   ...
   chapter 110 in-guest proxy prefix — which then 502'd
...
written in chapter 110, when "fetching anything outside the
guest" meant "send it to the in-guest httpd proxy". Three
chapters of native TLS work later (127 → 130) the proxy
...
Case (3) had been updated in chapter 127 to passthrough when
...
`scripts/test_browser_hn_*.py`) we keep the chapter 110
...
bundle anchors shipped in chapter 130, and renders. Same
...
chapter 121 already wrote for form actions. It does the
...
designed in chapter 110 to enforce one rule: "if it isn't
...
yet. `resolve_url` itself would need extending; chapter 121
...
- [`userspace/browser/browser.c`](../../../userspace/browser/browser.c) —
  ...
  since chapter 127, the guard just hadn't been cleaned up
...
This bug was latent for the entire 114 → 130 arc because:

- Stylesheets only get fetched for `http://` pages (chapter
  121 forms), and no test page exercised a no-path base.
- Image src resolution started using `resolve_url` (chapter
  98) but every page that had images had a path
  (`/mnt/test_layout.html`, the chapter 127 localhost test,
  ...).
...
That guard was a vestige from before chapter 127. Since
127 the fetch path is uniform: `fetch()` -> `http_fetch()`
...
The chapter 121 memory note already documented several
...
- [`scripts/test_browser_https.py`](../../../scripts/test_browser_https.py)
  (chapter 127) — PASS. Native TLS to `https://localhost:8443/`
...
- [`scripts/test_browser_https_multi.py`](../../../scripts/test_browser_https_multi.py)
  (chapter 128) — PASS. Multi-anchor TLS path unaffected.
- [`scripts/test_tls_pem_bundle.py`](../../../scripts/test_tls_pem_bundle.py)
  (chapter 129) — PASS. Bundle-format invariants unaffected.
- [`scripts/test_browser_proxy.py`](../../../scripts/test_browser_proxy.py)
  (chapter 110) — PASS. The critical proxy regression: with
...
- [`scripts/test_browser_self.py`](../../../scripts/test_browser_self.py)
  (chapter 111) — PASS. End-to-end loop through the in-guest
  proxy still closes.
- [`scripts/_dbg_bare_host.py`](../../../scripts/_dbg_bare_host.py)
  (NEW; `_dbg_*` prefix keeps it out of the regression
  sweep but inside the tree as a reference probe) — manual
...
Same shape as the chapter 130 and 121 limitations list:
