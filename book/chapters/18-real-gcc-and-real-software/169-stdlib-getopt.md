# Chapter 169 — `<stdlib.h>`: `qsort`, `bsearch`, `strtol`, `getopt`
...
> Prerequisites: chapter 167 (string / ctype / assert),
> chapter 149 (errno).
...
to chapter 171 — they return `double`, which the
...
2. **`strtol`, `strtoul`, `strtoll`, `strtoull`** — Doom's
   ...
   (chapter 167) is too lossy — it can't report invalid input
...
`-mgeneral-regs-only` build forbids. They land in chapter 171
...
passed"` marker pattern as `strtest` (chapter 167) and
`timetest` (168). The script boots the kernel, waits for
...
2. **C99 `<stdlib.h>` is an umbrella header.** Real glibc and
   ...
   150 additions, `<unistd.h>` if one ever ships) should
...
Chapter 170 — replace the chapter-19 minimal `printf` with
a real one. `%d`, `%u`, `%x`, `%o`, `%s`, `%c`, `%p`, `%lld`
plus width / precision / flag modifiers (`-`, `+`, ` `, `0`,
`#`). `%f`/`%e`/`%g` deferred to chapter 171. Adds `scanf`
for the `%d` and `%s` subset. Then chapter 171 turns FP on,
re-enables `strtod` here, and re-enables `%f` in 170's
`printf`.
