# Chapter 155 — A linker and an archiver: /bin/ld and /bin/ar
...
> into static archives. Builds on chapter 154 (`/bin/as`).

## Why this chapter exists

[Chapter 154](154-bin-as-assembler.md) gave us `/bin/as`,
...
That output is what the kernel ELF loader (chapter 21 +
...
`/bin/ld` yet — chapter 154's tests link individual
...
- The same `memset` / `memcpy` 5-liners to dodge
  the freestanding-memset trap (GCC will lower a
  large `= {0}` aggregate init into an implicit
  `memset` call, and our freestanding userspace has
  no libc to satisfy it).
...
none of which the assembler in chapter 154 can produce
...
chapter 154's assembler can emit:
...
(chapter 157 will), so the missing index is fine.
...
until chapter 155 became the first thing that
...
on (the persistence layer in chapter 85 already
has it; the userfs proxy in chapter 132 still
...
crt0 calls everything `_user_start` (chapter 21 ELF
...
network — and chapter 163's "Build" button is
...
[Chapter 156](156-crt0-and-libgcc-stubs.md) gives
...
…) so the next compiler (`/bin/cc`, chapter 157)
