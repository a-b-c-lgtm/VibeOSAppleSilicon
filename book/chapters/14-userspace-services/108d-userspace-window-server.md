# Chapter 108d — The window server moves to userspace

Chapters 108a, 108b and 108c each pulled one thing
out of the kernel: window pixel buffers, font
rendering, then per-app rasterisation. After 108c
...
input router — see [chapter 108e](108e-userspace-decoration-input-resize.md)
...
|---|---|---|
| 108a    | window pixel buffers, via shared mapping | rasterisation, fonts, compositor, decoration |
| 108b    | font rendering, via `/srv/font`          | rasterisation, compositor, decoration |
| 108c    | per-app rasterisation, via `libgui/draw` | compositor, decoration |
| **108d**| **the compositor**                        | **the input router and the window list** |
...
4. A client library (`libgui/wmclient`) that
   ...
   to `libgui/draw.h` in 108c only need a
...
1. **The trust boundary stops mattering for GUI
   ...
   transparently reconnects (chapter 108's
...
4. **It closes the loop on chapter 108b's
...
## Why this comes after 108c, not before

Two reasons:

1. **`wsd` needs `libgui/draw.h`** (chapter 108c)
   to draw its decorations and wallpaper.
   Without it, `wsd` would either re-implement
   rasterisation (duplicating what 108c
...
2. **Client apps need to already be using the
   ...
   Chapter 108c does the porting; this
   chapter inherits the result.

The ordering `108 → 108a → 108b → 108c → 108d` is
...
[chapter 108e](108e-userspace-decoration-input-resize.md)
...
`DESC_SW_WM_WINDOW` (chapter 108a's bit) so
...
chapter 107's `srv_bind`. The protocol is
defined in
...
* **Window-local damage coordinates.**
  ...
  from chapter 108c, so the client library
...
chapter 48's in-kernel WM, translated to
userspace.
...
* **`struct gui_fb` embedded as the first
  field.** Every chapter-108c drawing
...
chapter-108d call site: framed datagrams remove message
...
* No text rendering — that would pull in the
  `/srv/font` dependency from chapter 108b
...
predicate from milestone 15 stays:
...
* `CURSOR_BITMAP[]` — the hand-rolled X11
  left_ptr cursor sprite; the chapter 108e
...
pattern has paid off — see chapter 87's
...
atomically: the `/srv/wm` endpoint (chapter 107
...
* `wm_measure_text` and the kernel font
  ...
  the new path in chapter 108e, at which
...
chapter; chapter 108e takes a first step by
...
* Chapter 107 — IPC, the bus underneath
  `/srv/wm`.
* Chapter 108 — the supervisor pattern that
  keeps `wsd` respawning.
* Chapter 108a — `SYS_GUI_MAP_WINDOW`.
  Conceptually `win_fb` is its inverse: in
  108a the kernel allocated, the client
  mapped; in 108d a userspace daemon
  allocates, both the daemon and the
  owning client map.
* Chapter 108b — fontd. `wsd`'s eventual
  decoration text (chapter 108e) uses it
  via libgui.
* Chapter 108c — `libgui/draw.h`. `wsd`
  uses it to paint wallpaper and (in
  108e) decoration; clients use it for
  content.
