# Chapter 89 — IPIs via GICv3 SGIs

Chapter 87 brought up CPU 1 and parked it in WFE. Chapter 88
...
- Chapter 8 — GIC v3 distributor and CPU interface basics.
- Chapter 87 — secondary CPU bring-up via PSCI.
- Chapter 88 — atomics (the per-CPU receive counters).
...
Up to chapter 88 our `gic_init` was hardcoded for "the boot
...
Two vectors ship in chapter 89:
...
chapter 90's `IPI_RESCHED` will be the first to return 1.)
...
- IPI rate today is "approximately zero" (one ping per
  secondary at boot).
- Future RESCHED rate (chapter 90) will be at most a few
  per millisecond.

So chapter 89 ships the simpler scheme: **one SGI ID per
vector**. The receiver just reads `intid` from `ICC_IAR1_EL1`
and dispatches on it. If we ever measure SGI-storm pressure
in chapter 90 we can switch to the mailbox model — the
public `ipi_send`/`ipi_broadcast_others` API stays the same.

## Wake-from-WFI vs WFE

Chapter 87's secondary parked in `wfe`. That was wrong-shaped
for chapter 89: WFE only wakes on an explicit SEV (Send Event)
...
Choreography (matches the chapter 88 smoke pattern):
...
Like the chapter 88 smoke, this runs on every boot — and
...
- **No `IPI_RESCHED`.** It's the natural next vector but
  pointless until chapter 90 builds a runqueue that CPU 1
  can be kicked into.
- **No TLB shootdown.** Pointless until two CPUs share an
  address space (chapter 90 + the eventual mmap chapter).
...
- **No `panic_halt_others()` call from the panic handler.**
  ...
  to corrupt. Once chapter 90 has CPU 1 actively running
...
caller-side preconditions in chapter 90.
...
- **Chapter 90 — SMP scheduler.** Now that we can wake a
...
- **Future panic-halt-others.** The panic path will
  ...
  chapter 90 it becomes essential.
