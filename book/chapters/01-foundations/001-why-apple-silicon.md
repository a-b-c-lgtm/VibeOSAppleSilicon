...
userspace program in chapter 14 uses the same inline-asm syscall
stubs as the early kernel's UART driver. A real libc grows
organically — `printf` arrives in chapter 27 when the shell demands
...
- Patience for one slow chapter (chapter 6, the MMU) and one fiddly
  chapter (chapter 10, the context switch). Everything else is
