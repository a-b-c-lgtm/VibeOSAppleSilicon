# Chapter 115 — Moving font rendering into userspace

Chapter 104 added a TrueType rasteriser to the kernel.
...
chapter 113 told for the clipboard: features whose
...
The kernel rasteriser shipped in chapter 104 was the
...
* **`/bin/fontd`** — the font daemon.
  ...
  Binds `/srv/font` at boot (chapter 112). Owns the
...
* **`/bin/fontd` supervision in init.** Same
  pattern as chapter 113's `/bin/clipboardd`:
...
* Chapter 104 — the in-kernel TTF rasteriser. We're
...
* Chapter 112 — IPC. The font protocol is a
  textbook chapter-107 service.
* Chapter 113 — the supervisor pattern in init.
...
renders TTF text on a chapter-115 kernel.

Chapter 116 then takes the next step — adds a
...
chapter 104 is the regression. It boots,
...
chapter 113's clipboard supervisor regression
...
* Chapter 116 adds `libgui/text.h` for apps that
