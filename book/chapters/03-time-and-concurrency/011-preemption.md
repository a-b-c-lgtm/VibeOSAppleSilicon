# Chapter 11 — Timer-driven preemption
...
In the previous chapter [kernel/core/irq.c](../../../kernel/core/irq.c)
ended with
...
This chapter adds a single boolean and one call:
...
minimal AAPCS-callee-saved layout in chapter 10, this chapter
...
return path. We did not notice this one until much later — at
which point the symptom had been latent in the codebase for
months, firing roughly once every few hours of idle uptime.

### The symptom

After the browser landed, the desktop would sit happily until — at
...
## What chapter 12 already did

Chapter 12 introduced the kernel heap, which made
`thread_create` possible (each thread allocates a 16 KiB stack
and a struct from `kmalloc`). The book deliberately keeps
chapter 12 in part 4 ("userspace and storage") rather than
here, even though the heap is what unblocks chapter 10. The
ordering reflects the "story" of the build: heap was the last
thing the early kernel needed *before* threading became an option.

## What chapter 13 will need
...
A build of this chapter should produce the alternating output above
