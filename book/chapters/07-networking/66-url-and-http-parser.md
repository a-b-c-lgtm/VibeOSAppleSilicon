# Chapter 66 — URL and HTTP parser

[Chapter 65](65-dns-resolver.md) finished the network plumbing:
...
- A new **URL-form** for `/bin/httpget`: `httpget <url>`. The
  ...
  M56/M57 socket tests keep grepping the same byte stream.
...
- TLS. `https://` URLs are recognised, then politely refused.
  We will revisit TLS in a much later milestone — it needs
...
```text
httpget <host-or-ip> <port> [path]      # M56/M57 form
```
...
`THREAD_STACK_SIZE` in [Chapter 11](../03-time-and-concurrency/11-threads-and-context-switch.md));
...
## Bug surfaced by M58 #1 — the four-connection TCP cap
...
We had four TCP control blocks for the entire system. M55
allocated them carefully, M56 freed them in `socket_close`,
but the M57 boot self-test plus the M55 boot HTTP self-test
...
## Bug surfaced by M58 #2 — panic-handler recursion
...
[Chapter 7](../02-memory/07-physical-memory-and-device-tree.md)).
...
After the emergency stack landed, the original M58 panic
...
## Bug surfaced by M58 #3 — heap-bottom matters

A more pedestrian bug: the kernel heap allocator was searching
for free blocks from the *highest* address downward. M58
...
The browser begins in [Chapter 67](67-html-tokenizer.md):
the HTML tokenizer. The contract from M58's side is clean:
