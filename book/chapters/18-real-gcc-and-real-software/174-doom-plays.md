...
> for the cross-built Doom binary. Prerequisites: chapter 172
> (Doom port) and chapter 173 (WAD on disk).
>
> **Heads-up.** The 250-ms timed-release input shim used in
> this chapter is retired in chapter 197 and replaced by real
...
- **172** built the binary. DoomGeneric cross-compiled,
  linked against the chapter-165-onwards libc additions,
  loaded by the chapter-13 ELF loader, ran straight through
  `D_IdentifyVersion` and the FP/SIMD-at-EL0 paths added in
  chapter 171.
- **173** put the data next to the binary. `mkosfs2.py`
  ...
  chapter then fixed the in-game motion bug — chapter 29's
...
- **174** (this chapter) is the *acceptance* chapter: a
  ...
  one design decision left implicit by 172/173: where
  `/bin/doom` sits in the desktop's app surface (answer:
  it doesn't, yet — see "What 174 is **not**" below).

This chapter is short. The interesting work shipped in 172
and 173. What this chapter does is decide what "Doom plays"
...
- **Not a desktop-launcher integration.** A user playing
  ...
  more sprite; both are mechanical (chapter 30 and chapter
...
- **Not a Doom-from-GUI test.** A first cut of the script
  ...
  the chapter-117 WM is by *window-creation order*, not
...
- **Not a full input-driven regression.** The hardest
  thing the chapter-173 postscript fixed was a *gameplay*
...
## What 174 **is**: the regression contract
...
- WM regression breaks the chapter-117 damage path so the
...
unaffected because the chapter-117 WM grants focus on
...
This is the embarrassing part of Phase 1. The chapter-173
...
chapter-173 timed-release fix (step 8 — sustained motion),
the chapter-171 FP-at-EL0 path (step 6 — the episode-select
fade uses the renderer's float-heavy palette interpolation),
and the chapter-117 input plumbing end-to-end (steps 4–7).
...
In the strict apps-must-use-features sense, chapter 174 unlocks:

- **`/bin/doom`** is now the canonical exerciser of the
  chapter-173 timed-release input shim. Every menu Enter,
  every step forward, every shot fired is a unit test for
  that fix. (Existing chapter `_dbg_doom_input_timing.py`
  was the dev-time scope; chapter 174's manual procedure
...
- **The "Phase 1 done" marker** in the chapter 164 plan
...
Phase 2 begins with [175 — binutils target
setup](175-binutils-target.md): teach binutils about an
