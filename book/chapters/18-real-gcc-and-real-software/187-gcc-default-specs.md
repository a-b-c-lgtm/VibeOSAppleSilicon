# Chapter 187 — `gcc hello.c -o hello` (no escape-hatch flags)
...
> 0**. Prerequisite: chapter 186 (xgcc runs in the guest
> end-to-end).
...
Chapter 186 ended on step 7 of the test ladder: `/tmp/hello`,
...
1. Preprocess with the right `__OSDEV_LIBC__` define (LIB_SPEC
   side of `aarch64-osdev.h`, already in place since 184).
2. Run cc1 with `-ffreestanding` (CC1_SPEC, also already 184).
3. Run `as` to produce an `.o` (no spec involvement).
4. Drive `ld` with:
   - `-T /bin/linker_user.ld` (LINK_SPEC, 184).
   - `crt0.o` *as the first object* (STARTFILE_SPEC, 184).
   - `--start-group -losdevc --end-group` (LIB_SPEC, 184).
...
```
[chapter 187] /bin/gcc end-to-end smoke test
...
/* `-L /bin` is added so `-l<name>` lookups (from LIB_SPEC's
   ...
   /bin and will silently not match.  (Chapter 187.)  */
#undef LINK_SPEC
#define LINK_SPEC "                              \
   %{static:-Bstatic}                            \
...
real moment to wake up on the guest (chapter 186's stale-
...
of the book. From chapter 154 (assembler) through chapter
186 (xgcc runs in the guest) every piece arrived one at a
time. Chapter 187 is the rung where every piece is finally
on the wall.
...
- Compile any single-source C program that uses only
  ...
  ctype/stdlib (chapters 148-164). That's enough for the
...
- Build a multi-file program by compiling each `.c` to a
  ...
  invocation per TU; `make` arrives in 191.)

- Pipe `gcc` into the shell from inside notepad's build
  button (chapter 163). The placeholder PATH lookup from
...
- **Add `-I /bin` (the header equivalent).** Not needed —
  ...
  `-iprefix` chain through the chapter-186
...
- **Wire up `-lm`.** There's no separate `libm.a` — math
  ...
  no-op stub archive in 191 if any port asks for it; so
  far none have.

- **Wire up `-lpthread`.** Threads in osdev are a
  kernel-side feature (chapter 10, chapter 92) reachable
  from the libc via direct syscall wrappers. No POSIX
  `pthread_*` surface yet. When a chapter-191 port lays
...
helpers, and the env-arena rework from chapter 186 all
...
- `gcc hello.c -o hello` works as a user-facing command
  inside the OS shell.
- `notepad`'s build button (chapter 163) can now compile any
...
- The doom build (eventual chapter 134) and the first port
  in 192 have a real default-specs compiler to drive
...
- **Existing apps modified**: `userspace/notepad/notepad.c`'s
  build button (chapter 163) was already invoking `/bin/gcc`
...
187 is a one-trick chapter, but the trick is the one that
...
- Chapter 188 drives `gcc` against a real multi-source
  program built by the in-guest shell.
- Chapter 191 ports `make`, so user programs stop being
