# Chapter 41 — tmpfs polish: `>>`, `ls /tmp/`, `rm /tmp/...`

Chapter 40 introduced a writable filesystem at `/tmp/` and a shell `>`
...
file's current size — the `O_TRUNC` semantics in chapter 40 only kicked
...
`SYS_LISTDIR` in a loop since chapter 28 and just sees a longer list
now.  Output:
