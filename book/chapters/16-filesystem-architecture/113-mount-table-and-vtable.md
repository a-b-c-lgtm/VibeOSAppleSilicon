...
| ---------- | --------------------------------------- | ---------- |
| (root)     | embedded ramfs (`/motd`, `/README`)     | chapter 16 |
| `/mnt/`    | OSFS-1 (read-only on-disk)              | chapter 21 |
| `/bin/`    | OSFS-1 (binaries, same mount)           | chapter 22 |
| `/tmp/`    | tmpfs (in-memory, writable)             | chapter 41 |
| `/data/`   | OSFS-2 (writable on-disk)               | chapter 84 |
| `/proc/`   | procfs (synthetic, read-only)           | chapter 99 |
...
subdirectory support (chapter 85), so the `fs_ops` instance
...
The driver added in chapter 99 fits the vtable trivially.
...
- The fd_table refcounting in chapter 93.
