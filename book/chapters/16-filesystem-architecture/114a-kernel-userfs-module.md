# Chapter 114a — Step 1: the kernel half of userfs

[Chapter 114](114-userspace-filesystem-servers.md) committed
...
userspace until [chapter 114b](114b-sys-mount-umount.md) lands
...
land in 114b. If we hate the wire format tomorrow, deleting
...
`SYS_MOUNT` runs in chapter 114b. After that hand-off the
...
We'll relax this in
[chapter 114f](114f-timeouts-and-deadlock.md) when we add
...
The reason these two fields land *now* and not in 114b is
...
the GCC freestanding `= {0}` trap from
`/memories/freestanding-c-memset-trap.md`. We avoid it by
...
chunk loop lands in 114b alongside `SYS_MOUNT`.
...
table. The next chapter, [114b](114b-sys-mount-umount.md),
adds the two syscalls that make this code reachable.

We could have landed them together. The reason to split is
the same reason chapter 113 split the table from the first
...
- **Existing apps modified**: none. The `FD_USERFS_FILE`
  branch is unreachable until 114b.
- **New apps added**: none. Daemon-side comes in 114c.
...
- **Tests added**: none in this chapter. Step 1 has no
  ...
  114b/114c tests to exercise the protocol end-to-end.
