...
6. Define `__OSDEV_LIBC__` in the chapter 131b wrapper
...
Chapter 131c shipped the *cross-build seam* (link-mode
...
The remaining ~107 come from one of four classes the 131c
catalog identified:
...
The 131c plan called for a sweeping fix to class A:
...
([131e](131e-binutils-ld-in-guest.md)) is going to revisit
...
+/* osdev libc (chapter 131d) supplies optarg/optind/optopt/opterr/getopt
...
1. Nothing in `getopt1.c` conflicts with the OSdev libc
   today.
2. `binutils-the-tools` (gas, ld, ar — chapter 131e's
...
get `dev=0, ino=0` because chapter 131e doesn't need
...
The chapter 131b byte-identity test still passes
unchanged:
...
- **Chapter 131e:** with `libiberty.a` building, the
  ...
  Those binaries currently exist (from chapter 131a)
  but their `host` is the Mac; 131e cross-builds them
  for the guest so they can run inside QEMU.

- **Chapter 131f:** ship those guest binaries as
  `/bin/as` and `/bin/ld`, retiring the chapter 118/119
...
Chapter 131e host-builds the full gas + ld against the
