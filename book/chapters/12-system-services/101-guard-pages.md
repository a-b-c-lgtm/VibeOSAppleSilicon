...
[Chapter 27's argv/argc work](../05-devices/27-argc-argv.md) closed
...
* [Chapter 5 — Exception vectors, ESR, FAR](../02-memory/05-exception-vectors.md)
  for the EC / FAR / ISS decoding.
* [Chapter 24 — Per-process address spaces](../05-devices/24-per-process-address-spaces.md)
  for the user-stack VA layout.
* [Chapter 27 — argc/argv](../05-devices/27-argc-argv.md) for the
  original postscript that names this chapter as the fix.
* [Chapter 73 — fork](../09-process-model/73-aarch64-fork-and-as-copy.md)
  and [Chapter 75 — COW fork](../09-process-model/75-copy-on-write.md):
  the guard has to survive both clone paths.
* [Chapter 90 — mmap and the page cache](../11-smp-and-memory/90-mmap-and-page-cache.md)
...
* `DESC_SW_COW` (bit 55) — chapter 75, "this RO mapping is a COW
  share; on write fault, allocate and copy."
* `DESC_SW_PAGECACHE` (bit 56) — chapter 90, "this page belongs
  to the page cache; on AS teardown call `page_cache_release(pa)`
  not `pmem_dec_and_free(pa)`."

For chapter 101 we claim **bit 57**:
...
Chapter 101 adds a third check, **before either of them:**
...
for (int j = 0; j < PTES_PER_TABLE; j++) {
    ...
    if ((src_ent & DESC_VALID) == 0) {
        /* Chapter 101 — preserve guard pages across fork. */
...
1. **It silences the bug.** The whole win of chapter 101 is
   ...
   completely unrelated to the actual recursion. The M64
...
The M64 browser bumped `USER_STACK_PAGES` from 4 to 16 because
...
chapter 101, the same bug would have produced
