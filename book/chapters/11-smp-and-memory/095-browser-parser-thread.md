# Chapter 95 — The browser parser thread: HTML/CSS/layout off the GUI core
...
> 93 (shared fd tables) and chapter 93 (`CLONE_CPU`).

The chapter-93 work bought us a way to share open file descriptors
between two threads in the same address space. Chapter 95 cashes
...
stuck rectangle. Chapter 95 makes the GUI loop spin through
...
* **Chapter 93** — CPU pinning. The parser is spawned on
  CPU 1 specifically, so a hot resize loop on CPU 0 doesn't
  starve it.
* **Chapter 94** — CLONE_FILES. The parser is spawned via
...
chapter 96-and-beyond can have the parser thread do incremental
...
Before chapter 95 the userspace was strictly single-threaded
...
round-trip would be. The chapter 92 mutex is right for the
...
behavioural change than chapter 95 wants to ship.
...
happen — chapter 92-floor punted on it because there was
only ever one in-tree caller and the test was careful. With
chapter 95 there are two callers across the codebase, so
...
* **Image loading on the parser thread.** Future PNG / JPEG
  decode (chapter 11-system-services range) can run on the
...
* **Text-shaping on a worker.** TrueType + sub-pixel
  rendering (chapter 11) is the same shape of CPU-bound
...
* `book/chapters/11-smp-and-memory/095-browser-parser-thread.md`
  (this file)
