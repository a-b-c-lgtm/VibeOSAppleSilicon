# Chapter 91 — mmap and a unified page cache
...
- **Allocate large amounts of zero-filled memory cheaply.** The
  user-heap (chapter 16) hands out bytes from a fixed
...
- **Chapter 13** — the block cache. Same pattern, one layer
  ...
  the disk.
- **Chapter 16** — the user heap. mmap is the second source of
  user pages (after `sbrk`); both have to coexist in the same
  user VA layout.
- **Chapter 74** — copy-on-write. Two mechanisms (the
  `DESC_SW_COW` software bit, the AS-clone deferred-copy walk)
  carry over: chapter 91 introduces a *second* software-defined
...
USER_MMAP_BASE      0x10_3000_0000   ← chapter 91
USER_MMAP_MAX       0x10_3F00_0000   ← chapter 91 (240 MiB)
...
**permission fault** with WnR=1: the COW path from chapter 74.
A translation fault was always fatal — "you read garbage,
goodbye."

Chapter 91 splits the dispatch:
...
descriptor, identical pattern to chapter 74's `DESC_SW_COW`:

```c
#define DESC_SW_COW        (1ULL << 55)   /* chapter 74 */
#define DESC_SW_PAGECACHE  (1ULL << 56)   /* chapter 91 */
```
...
Chapter 91's deliberate floor: **mmaps do not survive fork**.
...
When chapter 92+ adds threading (or someone writes a forking
...
Chapter 90 declared the 24-test regression sweep "passes
...
running CPU. Chapter 91 had to surface this to honestly run a
regression sweep:
...
- **Shared text for ELF binaries.** When the loader uses
  ...
  proportional to instance count. (Chapter 92+.)
...
- **No `MAP_SHARED`.** Chapter 91 has the cache infrastructure
...
[mmap] file OK
[mmap] OK
PASS: chapter 91 mmap smoke test
```
