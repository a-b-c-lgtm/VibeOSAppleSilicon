# Chapter 113g — Step 7: `MOUNT_RO` + `EROFS_VFS` hardening
...
chapter 113f's user-visible payoff, this chapter delivers
...
clarity: the unsuffixed `EROFS` was used by chapter 18's
...
The full regression sweep from chapter 113e was rerun and
stayed green:
...
chapters 113a through 113g — is sweep-green and the prefix
ladders are gone.
...
- **The `MOUNT_NODEV`, `MOUNT_NOEXEC`, `MOUNT_NOSUID` bits.**
  Chapter 113a reserved bits 1..31 for them. None of them
...
- **The legacy `sys_listdir`.** Chapter 113c noted that the
...
read-only contract is enforced uniformly. Chapter 114 picks
...
work is unchanged in shape from the chapter 113 plan; the
...
- New apps added: none — the user-visible payoff was in
  113f's `/bin/mount`. 113g is the invariant tightening
