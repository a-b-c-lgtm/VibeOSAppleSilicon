# Chapter 186 — `gcc hello.c` works on the OS
...
> 8 / FAIL 0. Prerequisites: chapters 179 (in-guest `ld`),
> 180 (real `/bin/as` + `/bin/ld`), 183 (cross-built xgcc),
> 184 (real specs file), 185 (gmp / mpfr / mpc in the guest
> sysroot).
...
By the end of chapter 185 everything required to **link** a
guest gcc existed under `build/gcc-build-guest/`. What
chapter 185 did **not** verify is that the resulting `xgcc`
...
as          11,XXX,000 B    binutils-2.44 assembler (ch 180)
ld           7,XXX,000 B    binutils-2.44 linker    (ch 180)
...
```
[chapter 186] /bin/gcc end-to-end smoke test
...
```c
  /* osdev chapter 186: when no implementation was selected
...
The driver patch from chapter 184
...
hello.c` finds the right crt0 / libc is chapter 187's
problem.
...
- The doom build (chapter 192 — port `make` and then build
...
- **Existing tools that use the new compiler**: `notepad`
  ...
  via the gui_term shell. Chapter 163's "build button"
  prototype was a placeholder; chapter 186 wires it to a
  real compiler. (See `userspace/notepad/notepad.c::on_build`
  — the placeholder PATH lookup will go away in chapter 187
...
- Chapter 187 wires up default specs so a bare
  `gcc hello.c` finds the right crt0 / libc instead of
  needing `-nostdlib -nostdinc -e _start -Wl,-T,...` on
  the command line.
- Chapter 191 ports `make` into the guest so larger
