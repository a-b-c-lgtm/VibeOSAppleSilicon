...
| Chapter | Brought us |
|---------|------------|
| [105](105-bin-httpd.md)   | `/bin/httpd` -- the server itself |
| [106](106-tcp-loopback.md) | `127.0.0.1` as a real address |
| [106a](106a-httpd-tls-bridge.md) | httpd as a transparent forwarder |
| [106b](106b-browser-uses-in-guest-httpd.md) | `BROWSER_PROXY` repoint + the eight-bug postscript |
...
- [Chapter 105](105-bin-httpd.md) -- `/bin/httpd`.
- [Chapter 106](106-tcp-loopback.md) -- loopback short-circuit
  in `ip_send_packet`.
- [Chapter 106a](106a-httpd-tls-bridge.md) -- the HTTPS
  forwarding bridge (not used by this chapter directly, but
  the architecture it locked in is why port-80 + port-8080
  can coexist).
- [Chapter 106b](106b-browser-uses-in-guest-httpd.md) -- the
  browser's default-proxy plumbing and the eight bugs that
  the postscript documents.
- [Chapter 63](../08-browser/63-browser.md) -- the browser.
...
requests via the chapter-105 `serve_get` / chapter-106a
...
- [`userspace/proxytest/proxytest.c`](../../../userspace/proxytest/proxytest.c)
  ...
  `load_proxy_from_env` path chapter 106b introduced.
...
[`test_browser_hn_serial_with_desktop.py`](../../../scripts/test_browser_hn_serial_with_desktop.py),
which routes through the chapter-106a forwarding proxy.
...
[chapter 106b postscript](106b-browser-uses-in-guest-httpd.md#postscript):
...
[chapter 99](../12-storage/99-procfs.md) procfs file --
...
Following the
[apps-must-use-features](../00-prologue/apps-must-use-features.md)
discipline -- every kernel feature has to land in an app
...
| [`book/INDEX.md`](../../INDEX.md) | mark milestone 98 done |

## What chapter 106c does not do
...
- **A multi-page sample site under `/mnt/www/`.**  The
  ...
  and link-following (chapter 64 and on), so we could
...
- **A framebuffer-diff regression that compares the
  ...
  have the building blocks (chapter 98 PNG decode could
...
- **HTTPS termination at httpd.**  Chapter 106a forwards
