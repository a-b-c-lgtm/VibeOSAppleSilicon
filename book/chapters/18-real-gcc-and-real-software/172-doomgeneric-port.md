# Chapter 172 — Doom (the port)

> **Milestone in this chapter:** vendor DoomGeneric and write the
> osdev backend shim that drives it through the chapter-117
> window manager.
> **Code referenced:**
...
> Prerequisites: chapters 165–170 (the libc surface), 171 (FP /
> SIMD at EL0).
...
*with* `%f` (170); ctype/assert/string (167); `time_t` and
friends (168); getopt and `qsort`/`bsearch` (169); finally
eager FP at EL0 (171). Each chapter ended with a small test
...
(chapter 117, `userspace/libgui/wmclient.h`), maps the WM's
...
- **`wm_window_dirty` is a hint, not a side effect.** Chapter
  117's compositor doesn't read the framebuffer until
...
(chapter 117's `wmclient.h` documents BGRA). No swizzle, just
a row copy.
...
The window manager (chapter 29 input multiplexing, refined in
117) currently only sends *press* events on the bus.
...
`sleep_ms` (chapter 28) rounds up to a scheduler tick (100 ms).
Doom asks for `1` ms a lot; it gets 100 ms. Frame rate caps at
~10 Hz in this build. Chapter 173 will introduce a finer
sleep wrapper (cycle counter? hrtimer?); for now the slow tick
is fine for a "does it run" demo.

`uptime_ms` is already 1 ms-resolution (chapter 20).
...
(chapter 14), so there's plenty of headroom.
...
tick (the only sub-tick option until 173). `system` is stubbed
...
since chapter 12 has produced this warning.
...
(173) will stage the WAD into `/data/wads/` on the runtime
...
- **Chapter 171 (FP/SIMD at EL0).** The renderer's
  ...
  chapter 171's eager save/restore, `doom` would die with
  `EC=0x07` (SIMD/FP access trap) on the second context switch
  after `D_DoomMain`.

- **Chapter 165 (setjmp/longjmp).** Doom uses `setjmp` for
  the long-distance error escape from `I_Error`. With FP at
  EL0 enabled, `setjmp` correctly saves `d8..d15` per AAPCS64
  — code touched in 165, observable for the first time here.

- **Chapter 167 (string/ctype).** Half of DoomGeneric is
  string parsing for the IWAD scanner and the config-file
  reader.

- **Chapter 168 (time).** The renderer ties its tick counter
  to `uptime_ms`; the demo recorder uses wall-clock `time(NULL)`
  for timestamps.

- **Chapter 169 (qsort/bsearch/getopt).** `qsort` is used by
...
- **Chapter 117 (userspace WM).** Doom is the largest single
