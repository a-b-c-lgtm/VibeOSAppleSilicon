# Chapter 62 — ICMP, UDP, and DHCP

[Chapter 61](61-ethernet-arp-ipv4.md) ended with a stack that
...
tests in chapter 61 unchanged.

## RX dispatch when you have no IP

The IPv4 receive validator in chapter 61 dropped any frame whose
...
chapter 61's IPv4 code computes — but with one important
difference.
...
client, milestone 55), and a couple of slots free for
experiments.
...
- **DHCP renewals.** We use `LEASE_TIME` for nothing yet — the
  ...
  effectively never expires. A renewal timer goes in milestone
  56 alongside the timer-driven scheduler interactions.
- **A proper DNS resolver.** We parse option 6 and stash the
  servers, but nothing dials them yet. Milestone 55 will add a
...
job of TCP, in [chapter 63](63-tcp-and-sockets.md).
