...
The register subset we need for milestone 0 is tiny:
...
we do need to program the device (mainly chapter 21, when we add
...
returns, the stub falls through into a `wfe` halt loop. For
milestone 0 it will not return — `kernel_main` enters its own
...
and the banner explicitly says "deferring DTB hex-dump to
milestone 1". This is a real constraint we hit during milestone 0,
...
exists, and milestone 1 (chapters 4–6) is what finally retires the
...
## What you have at the end of milestone 0
