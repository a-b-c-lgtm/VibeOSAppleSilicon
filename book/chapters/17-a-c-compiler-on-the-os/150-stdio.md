# Chapter 150 — `FILE *`, fopen, fread, fwrite, fseek, fprintf

> **Milestone in this chapter:** add the buffered stdio layer
> every libc consumer from chapter 153 onward assumes is in
> place.
> **Code referenced:**
...
> [Chapter 148](148-libc-stdio-and-env.md); follows
> [Chapter 149](149-errno.md).
...
1. **Per-thread errno.** Today `__errno_value` is a single
   ...
   move `__errno_value` into the TLS area, which chapter 152
...
2. **printf retargeting.** `printf` still writes straight to fd 1
   ...
   152 alongside the convention flip so we only touch each app
   once.
...
[chapter 150] FILE * layer (fopen/fread/fwrite/fseek/...)
...
- **Existing app rewrites:** deferred to chapter 152 (the
...
- **Header-only `FILE *` is fine for now.** The single-TU pattern
  ...
  proper `libc.a` static library, which is on the chapter 156
  roadmap.
...
[Chapter 151](151-env-arena.md) — owning `environ[]` arena so
