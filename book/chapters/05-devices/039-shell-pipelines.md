# Chapter 39 — Shell pipelines: `cat | grep | wc`
...
`SYS_SPAWN_REDIR` (chapter 36) was the natural place to grow,
...
Chapter 37 introduced `daifclr`/`daifset` around `thread_sleep_ms`
...
- The networking layer (chapter 38 in the next book) can use the
...
- The `<` redirection from chapter 36 and the `|` pipeline
