# Chapter 164 — Real GCC on the OS, and a playable Doom
...
> chain that determines the order chapters 165 through 197
> appear in. **No code lands in this chapter** — the
> implementation work begins in chapter 165.
...
- `/bin/gcc` — a real, in-guest C compiler that handles a
  realistic C codebase end-to-end. **Not** the chapter-157
  `/bin/cc` (deliberately tiny) and **not** a re-invention.
- `/bin/as` and `/bin/ld` — real GNU binutils, replacing the
  chapter-154/155 toys for any input larger than the
...
1. **Bootstrap.** Get GCC's source to compile through some
   existing in-guest compiler (the chapter-157 `/bin/cc`,
...
- **The cross-toolchain machinery is already in place.**
  Chapter 158 wrote down the cross-toolchain contract every
  userspace binary already respects. Adding one more
  cross-built binary (`aarch64-osdev-gcc`) is the same
  pattern Part XV used to ship BearSSL (chapter 124) and
...
This is exactly the [BearSSL](../15-browser-maturation/124-bearssl-build.md)
...
This is **chapter 171** and it must land before the
cross-built GCC binary in chapter 184. Until 171 ships,
Phase 1 (below) is the only thing the section can
demonstrate.
...
**165. `<setjmp.h>` and aarch64 setjmp/longjmp** — 50-ish
...
**166. `<signal.h>` libc wrappers** — `signal()`,
...
**167. `<ctype.h>`, `<assert.h>`, missing string fns** —
...
**171. FP/SIMD at EL0** — kernel change. Set
...
**172. Doom-on-host cross-build** — vendor DoomGeneric
...
**173. DOOM1.WAD on disk** — extend `mkosfs2.py` to stage
...
**174. Doom plays** — boot to desktop, terminal,
...
> **Done**: chapter 174 landed with the WAD-load + render
...
**175. binutils target setup** — define the
...
> **Done**: see
> [175 — Binutils with an `aarch64-osdev` target](175-binutils-target.md).
...
**176. binutils as in-guest libc shim** — the cross-built
...
> **Done (re-scoped).** The original 176 framing —
...
> chapter 181. So 176 was split:
>
> - **176 (delivered):** build the seam itself —
>   `aarch64-osdev-cc`, a 30-line wrapper around
>   `aarch64-elf-gcc` that adds
>   `-B build/toolchain/bin/` (for the chapter 175
...
>   directly. See `176-osdev-cc-wrapper.md`.
> - **177 (delivered):** taught the wrapper a link mode
...
>   handed off to 178. See
>   `177-cross-build-seam.md`.
> - **178 (delivered):** *not* the libc.a extraction
...
>      survives for binutils-the-tools in 179.
...
>   176 byte-identity baseline still passes (wrapper's
...
>   `178-libc-gaps.md`.
>
> Net effect on the plan: the original chapter-131 plan
> grew into a 5-chapter sub-series (176–180) instead of 4.
> Old "binutils ld in-guest" landed as 179; old "replace
> /bin/as and /bin/ld" landed as 180. No change in milestones.

**179 (delivered).** Cross-build binutils' `ld` for
> `aarch64-osdev`. Five companion archives configure +
> build under the chapter-177 wrapper (`libiberty` was
> 178; `libsframe`, `bfd`, `opcodes`, `libctf` join here);
...
>      `strdup` for the chapter 172 Doom port) with a
...
> `OSDEV_LIBC_NO_GLOBAL_DEFS` guard from chapter 172
...
>
> Host smoke
> [scripts/test_guest_ld.py](179-binutils-ld-in-guest.md)
> passes; chapter 178's
> `scripts/test_guest_configure.py` baseline still
> passes (libiberty.a now 949,790 bytes — modest growth
> from struct stat's chapter-178 `st_dev`/`st_ino`
> fields rippling into a couple of libiberty TU sizes).
> See `179-binutils-ld-in-guest.md`.

**180. Replace `/bin/as` and `/bin/ld`** *(done, alongside 179)* —
the OSFS entries `/bin/as` and `/bin/ld` now point at
`build/userspace/binutils/{as,ld}.stripped.elf` (the
binutils 2.44 `as-new`/`ld-new` built by 179, stripped to
~3 MB each).  The toy chapter-154/155 sources under
`userspace/as/` and `userspace/ld/` are no longer
packaged. `/bin/ar` is still the chapter-155 toy because
...
contract from chapter 158 is honoured.

**181. GCC target setup** — define `aarch64-osdev` as a
...
**182. GMP/MPFR/MPC** — GCC 14 (= the host version) needs
...
> Chapter **186 shipped** as the end-to-end hello-world
> smoke (`/bin/gcc` running inside the guest, synthetic
> `-nostdlib -e _start` link).  Chapter **187 shipped**
> the default-specs link (`gcc hello.c -o hello` with no
> escape-hatch flags).  See chapters 185, 186, and 187
> for full details.

**185. GMP/MPFR/MPC for the guest sysroot** *(done)* —
...
**186. `/bin/gcc` works on the OS** *(done)* —
...
**187. `gcc hello.c -o hello` with default specs** *(done)*
...
`scripts/test_gcc_bf.py` (PASS 6/6).  See chapter 188
for the header-shipping discussion and the host/guest
link-line symmetry technique.

**189. `#include <stdio.h>` works in the guest**
...
yet (see 190 below).  Test:
`scripts/test_gcc_sys_stat.py` not yet relevant here.
Fix: `scripts/test_gcc_stdio.py` (PASS 7/7); chapters 187
and 188 regressions both still green.  See chapter
189 for the directory-cap math, the "echo-staging
breaks on `<>`" trap, and the GCC-freestanding-headers
discovery.

**190. `sys/` headers without a hierarchical FS**
...
**191. `/bin/tar`** *(done)* — read-mode ustar reader
...
write-up in
[191 — `/bin/tar` (ustar reader)](191-tar.md).

**192. `/bin/make` audit against Doom's real Makefile**
*(done)* — chapter 162's 351-LoC `/bin/make` handled
only `target: deps + recipe`. 192 expands it to ~720 LoC,
...
Makefile for chapter 193 is hand-written with an explicit
`OBJS = ...` list. `scripts/test_make_v2.py` is
**PASS 9/9**; chapter-162 `test_make_port.py` remains
**14/0**; six earlier regressions stay green. Full
write-up in
[192 — expanding `/bin/make`](192-make-expansion.md).

### Sub-part D: end-to-end

**193. Rebuild Doom in-guest, pilot** *(done)* —
...
[193 — in-guest Doom rebuild pilot](193-doom-pilot.md).

**194. Rebuild Doom in-guest, full** *(done)* —
...
Surfaced two traps: (1) the chapter-192 `/bin/make`
...
[194 — in-guest Doom rebuild, full vendor compile](194-full-doom-compile.md).
Still .o-only — link comes in 195.

**195. Link `doomgeneric.elf` in-guest** *(done)* —
...
`expandargv`, free from chapter-180's real ld) rather
...
25-minute 194 compile. `scripts/test_doom_link.py` is
...
`icon.o` were dropped during 194) — fix to 194's
fixture queued for 196; (D) chapter-155's RWX-segment
...
[195 — in-guest Doom link](195-link-doomgeneric-in-guest.md).

**196. Rebuilt Doom plays** *(done — closes Part XVIII)* —
the in-guest-linked `/data/doomgeneric.elf` (chapter 195)
...
matches chapter-174's cascade math).
...
the cross-built reference (chapter 172) is NOT asserted
...
[196 — the rebuilt Doom plays](196-rebuilt-doom-plays.md).

The chapter-164 end-state — `tar` + `make` + `./doom`
...
1. **FP-at-EL0 (chapter 171)** has subtle interactions
   with the context switch (SP_EL0 handling, the
   IRQ-window discipline, SMP). Could blow up into 3
   sub-chapters.
2. **The libc-gap long tail (chapters 176, 184)** is
...
- **Floating-point-heavy compute** (numerical software,
  ML, audio synthesis). Chapter 171 turns FP *on*; it
...
- Part-XIX-class projects become *trivial* to scope:
  ...
  for BearSSL.
- Notepad's chapter-163 Build button could shell out to
...
prerequisites: none net-new; rides on
               chapter 91 (mmap), chapter 76 (sigaction),
               chapter 96 (RTC), chapter 158 (cross-toolchain
               contract).
next chapter:  165 (setjmp/longjmp)
```
