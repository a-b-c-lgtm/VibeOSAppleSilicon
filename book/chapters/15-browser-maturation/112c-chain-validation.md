...
`br_ssl_client_init_full` wires in by default — in 112b we
...
- `scripts/test_tls_chain.py` — boots the kernel, runs
  ...
  `/bin/httpsd` that init.c already spawns for 112b, asserts
  four progress signals.
- `book/chapters/15-browser-maturation/112c-chain-validation.md`
  (this file) + the corresponding `book/INDEX.md` rows.

`/bin/httpsd` is unchanged from 112b — it still presents the
...
`br_x509_decoder_*` API we already used in 112b to pull the
...
the same way 112b copies the pinned key (the decoder hands back
...
This is the first user of `SYS_GETTIMEOFDAY` (chapter 95)
...
if (t->chain_mode) {
    ...
    /* knownkey mode (112b): override the validator. */
...
the 112b draft) — it doubles as the dead-init scratch in
...
tls handshake: PASS chapter 112c end-to-end
```
...
| `tls handshake: PASS chapter 112c`             | App-data round-trip survived chain validation       |
...
2. **`append_dn` writing past the buffer.** Real-world DNs go
   ...
   Chapter 112d will lift this to a larger buffer (or to a heap
   allocation sized by the cert) when we start trusting real
   public CAs.
3. **Decoder pointer aliasing.** Same trap as 112b: the bytes
...
None of these bit us during 112c — they all came out of careful
...
- `/bin/tlstest` now has a second handshake mode that uses the
  ...
  - `tlstest --handshake 127.0.0.1 8443` — pinned (112b).
  - `tlstest --handshake-ca 127.0.0.1 8443` — chain (this
    chapter).
- The same path will be used unchanged by the browser in
  chapter 112d. The only difference there is the anchor list:
...
- `/bin/date` is no longer the only userspace consumer of the
  PL031 RTC. Cert expiry now depends on it; chapter 112d's
