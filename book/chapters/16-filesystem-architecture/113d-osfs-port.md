# Chapter 113d — Step 4: porting OSFS-1 and OSFS-2

The two on-disk filesystems in the tree are the biggest single
step in the chapter-113 sequence. OSFS-1 (chapter 21) is the
...
that out — chapter 13's exec path already called
...
Chapter 113e collapses that fallback when the root ramfs
becomes a mount too.
...
OSFS-2 has real subdirectories (chapter 85), so the adapter
...
- The `/data/` and `/tmp/` legacy ladders in `sys_unlink`
  (the chapter-113c port already cut the `/tmp/` one for
...
worth noting because chapter 113e does the same trick for
...
exactly as procfs did in 113b. The OSFS-2 open path is the
...
walker would dereference garbage. The memory rule from
`/memories/freestanding-c-memset-trap.md` is the gotcha to
remember here: an explicit per-field clear is the cheapest
way to do this in freestanding C without dragging in a
synthetic `memset` from the compiler's optimiser.
...
- `test_fsync.py` — exercises the OSFS-2 write path's
  durability contract from chapter 82.
...
RO check (added in 113g) was firing inappropriately, or the
...
- `scripts/test_clock.py` — `load` adapter for OSFS-1
  (`/bin/date`)
- Regression sweep — full chapter 21 through chapter 99
  coverage
