# Chapter 74 — Copy-on-write: making fork cheap
...
Eager-copy fork (chapter 72) costs O(allocated pages) per
...
- TLB: invalidate the just-touched VA on the current CPU.
  (TLB shootdown across CPUs comes in chapter 88.)
...
that are worth recording explicitly so chapter 90 (`mmap`)
...
read-only from EL1 too. So the moment chapter 74 marked
