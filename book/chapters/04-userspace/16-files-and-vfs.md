# Chapter 16 — Files, VFS, and a tiny ramfs
...
[Chapter 15](15-elf-and-first-user-program.md) gave us a user
...
2. **The chapter that adds a disk-backed FS lands without
   touching the syscall layer.** When chapter 18 (or wherever
...
That said, our milestone-8 VFS is *minimal*: there is no
...
- **Filenames embed the leading `/`.** No path walker means we
  match the whole string verbatim. When chapter 17 adds a
...
- **The fd table lives on the thread, not on a separate "process"
  object.** We don't have processes yet. When chapter 17
...
{
    /* TODO(milestone 9): copy_from_user with bounds check + len cap. */
...
strings. Chapter 17, when it splits address spaces, has to add
...
- **Reading from `console` (fd 0) returns 0 immediately.** That's
  ...
  keyboard. Chapter 22 (virtio-input) wires the keyboard ring
...
That single block exercises every milestone-8 path:
...
- **Directories.** There is one flat namespace. No `opendir`,
  no `getdents`. Chapter 18's disk-backed FS introduces a real
  hierarchy.
...
Next chapter: [fork, execve, wait4, exit](17-fork-execve-wait4-exit.md)
(coming with the next milestone, which also splits each process
into its own L1 page table).
