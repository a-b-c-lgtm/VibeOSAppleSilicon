...
The binary itself exercises plain double arithmetic at EL0, FP
register preservation across cooperative yields, and d8..d15
preservation through setjmp / longjmp (AAPCS64 callee-saved).

Same shape as test_libc_printf2.py.
"""
import os
import select
import socket
...
def main():
    ...
    try:
        ...
        if b"PANIC" in log:
            print("FAIL: guest panicked during fptest")
            print(log[-2000:].decode("ascii", "replace"))
            return 1
        if b"all checks passed" not in log:
            print("FAIL: fptest didn't reach the success marker")
            print(log[-2000:].decode("ascii", "replace"))
            return 1
        if b"FAIL " in log or b"FAIL:" in log[:log.rfind(b"all checks passed")]:
            print("FAIL: fptest reported individual CHECK failures")
            print(log[-2000:].decode("ascii", "replace"))
            return 1
        print("fptest: PASS")
        return 0
